	...
	implements View
{
	...
	private StringBuffer xlsfo;
	private String font;

	public PdfView()
	{
		xlsfo = new StringBuffer();
	}

	public void beforeBody(TableModel model)
	{
		font = model.getPreferences().getPreference("exportPdf.font");
	...
	public String regionBefore(TableModel model)
	{
		...
		String headerBackgroundColor = export.getAttributeAsString("headerBackgroundColor");
		sb.append(" <fo:static-content flow-name=\"xsl-region-before\"> ");
		String title = export.getAttributeAsString("headerTitle");
	...
	public String columnDefinitions(TableModel model)
	{
		...
		double colwidth = 10D / columnCount;
		for (int i = 1; (double)i <= columnCount; i++)
	...
	public String header(TableModel model)
	{
		StringBuffer sb = new StringBuffer();
		Export export = model.getExportHandler().getCurrentExport();
		String headerColor = export.getAttributeAsString("headerColor");
		String headerBackgroundColor = export.getAttributeAsString("headerBackgroundColor");
		...
		for (Iterator iter = columns.iterator(); iter.hasNext(); sb.append(" </fo:table-cell> "))
		{
