...
function eXcell_math(cell) {
	...
	this.edit = function () {
		...
		this.grid.editor.getValue = function () {
			return this.cell.original || this.getTitle();
		};
...
dhtmlXGridObject.prototype._calcSCL = function (cell) {
	...
	if (this._roundD) {
		...
		if (this._roundDl > 0) {
			...
			/**************************修改过,除法运算时处理分母为零的情况****************************************/
			if (".0NaN"==(pre + z.substring(0, n) + "." + z.substring(n, z.length)) || "Infi.nity"==(pre + z.substring(0, n) + "." + z.substring(n, z.length))){
				return "";
			}
			/**********************************************************************************************/
...
dhtmlXGridObject.prototype._compileSCL = function (code, cell) {
	...
	cell._code = code;
	/**************************修改过,除法运算时处理分母为零的情况****************************************/
	if (".0NaN"==this._calcSCL(cell) || "Infi.nity"==this._calcSCL(cell) || "Infinity"==this._calcSCL(cell)){
		return "";
	}
	/**********************************************************************************************/
