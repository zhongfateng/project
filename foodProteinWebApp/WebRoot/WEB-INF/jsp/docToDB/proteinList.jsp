		...
		for(i = 0 ; i < objIds.length; i++){
			if(objIds[i].checked == true){
			   if(confirm('确定要删除该条记录吗?')){
					...
					document.all.frm1.action = "protein.action?m=delete";
...
function moddata(){
	...
	if(objIds!=null){
		for(i = 0 ; i < objIds.length; i++){
			if(objIds[i].checked == true){
				document.all.frm1.objectId.value = objIds[i].value;
				document.all.frm1.action = "protein.action?m=toEditPage";
				document.all.frm1.submit();
				break;
			}
		}
...
<table cellpadding="0" cellspacing="0" width="100%" border="0" align="center">
						...
						<table border="0" cellspacing="0" cellpadding="0" align="right">
					     <tr style="height: 21">
							<td background="resources/skins/default/images/button/bu-bg1.gif" align="center" style="width: 64">
							   <a href="#" onclick="javascript:adddata('protein.action?m=toAddPage');">
							      添加
							    </a>
							</td>
							<td style="width: 3">&nbsp;</td>
	...
	<tr>
		...
		<ec:table 
			items="dataList"
			action="protein.action?m=loadData"
			...
			<ec:row>
					<ec:column property="id" title="选择"><input name="chkid" type="checkbox"  value="${m.id}"/></ec:column>
					       <ec:column property="ptype" title="ptype"/>
					       <ec:column property="chromosomeRef" title="chromosomeRef"/>
					       <ec:column property="chromosomeStart" title="chromosomeStart"/>
					       <ec:column property="chromosomeEnd" title="chromosomeEnd"/>
					       <ec:column property="ncbiProteinRef" title="ncbiProteinRef"/>
					       <ec:column property="note" title="note"/>
					       <ec:column property="geneId" title="geneId"/>
					       <ec:column property="speciesId" title="speciesId"/>
					       <ec:column property="ncbiSpeciesId" title="ncbiSpeciesId"/>
					       <ec:column property="sequence" title="sequence"/>
			</ec:row>
		</ec:table>
		</td>
	</tr>
