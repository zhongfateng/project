...
function ff(){
	...
	var filepath=encodeURI(filepath);
				...
				if(filepath==""||speName==""){
					...
					$.ajax( {
					"url" : "gene.action?m=toPaseGFF&gffPath=" + filepath +"&speName="+speName,
					"type" : "post",
					"dataType" : "json",
...
function fff(){
	if($('#dddpt').html()=="正在处理请稍等'''''''"){
		ymPrompt.alert('正在执行！');
	}else if($('#tipssqq').html()=="恭喜SQL语句执行完成:"){
		ymPrompt.alert('执行完成！');
	}else if($('#tipsses').html()=="您所处理的信息:"){
		$.ajax( {
					"url" : "gene.action?m=toSQL",
					"type" : "post",
					"dataType" : "json",
					"beforeSend": function(XMLHttpRequest){  
					...
					"success" : function(data, desc1) {
						$('#tipss').remove();
						$('#d2_3').after("<tr id='tipss'><td id='tipssqq' height='30' colspan='2' class='td_nbw_left' align='center'></td><td id='dd' style='color: red'></td></tr>");
						$('#tipssqq').html(
								"恭喜SQL语句执行完成:");
						$('#dd').html(
								"更新gene条数:" + data[0]);
				...
				<table width="100%" align="center" class="table_nbw" border="0">
					...
					<tr class="tr_nbw">
						...
						<td height="30" class="td_nbw_right" colspan="2">
							<input type="text" id="d2_1" name="speName"
								size="40%" />
					...
					<tr class="tr_nbw" id="d3">
						...
						<td height="30" class="td_nbw_right" colspan="2">
							<input type="text" id="d2" name="gffPath" 
								size="40%" />
					...
					<tr class="tr_nbw" id="d2_3">
						...
						<td height="30" class="td_nbw_right" colspan="3">
							<input id="d2_2" type="button" value="执行SQL" onclick="fff();"
								size="10%" />
							<span id="isbnTip" style="width: 200px"></span>
						</td>
					</tr>
