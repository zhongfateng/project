		...
		for(i = 0 ; i < objIds.length; i++){
			if(objIds[i].checked == true){
			   if(confirm('确定要删除该条记录吗?')){
					...
					document.all.frm1.action = "techDy.action?m=toDelTd";
...
function moddata(){
	...
	if(objIds!=null){
		for(i = 0 ; i < objIds.length; i++){
			...
			if(objIds[i].checked == true){
				document.all.frm1.objectId.value = objIds[i].value;
				document.all.frm1.action = "techDy.action?m=toModifyTd";
				document.all.frm1.submit();
				break;
			}
		}
...
<table cellpadding="0" cellspacing="0" width="100%" border="0" align="center">
					     ...
					     <tr  style="height: 21">
							<td>
							  <img src="${skinPath}images/button/button_1/zj.gif"
															style="cursor: pointer" onclick="javascript:adddata('techDy.action?m=toEditDy');">
	...
	<tr>
		...
		<div style="overFlow: auto; height: 550px">
						<ec:table items="dataList" action="techDy.action?m=toFileUp"
							imagePath="${skinPath}/images/table/*.gif" width="100%"
							rowsDisplayed="15" var="m">
							<ec:row>
								<ec:column property="tid" title="选择" width="10%">
									<input name="chkid" type="checkbox" value="${m.tid}" />
								</ec:column>
								<ec:column property="title" title="标题" width="20%">
									${m.title}
								</ec:column>
							</ec:row>
						</ec:table>
