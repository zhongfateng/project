...
<script type="text/javascript">
	function changepage(pid){
	    if(pid=='1'){
	    	window.location="search.action?m=fronttoload&searchWord=<%=request.getAttribute("searchWord")%>&page=1&pageSize=${pageable.pageSize}";
	    }
	    if(pid=='2'){
	    	window.location="search.action?m=fronttoload&searchWord=<%=request.getAttribute("searchWord")%>&page=${pageable.page-1}&pageSize=${pageable.pageSize}";
	    }
	    if(pid=='3'){
	    	window.location="search.action?m=fronttoload&searchWord=<%=request.getAttribute("searchWord")%>&page=${pageable.page+1}&pageSize=${pageable.pageSize}";
	    }
	    if(pid=='4'){
	   		window.location="search.action?m=fronttoload&searchWord=<%=request.getAttribute("searchWord")%>&page=${pageable.maxPages}&pageSize=${pageable.pageSize}";
	    }
	}
</script>
<script type="text/javascript">
		...
		if(page=="" || isNaN(page) || page<=0 || page>${pageable.maxPages}){
			...
			window.location.href="search.action?m=fronttoload&searchWord=<%=request.getAttribute("searchWord")%>&page="+page+"&pageSize=${pageable.pageSize}";
		}
...
<script type="text/javascript">
		...
		function  changeone()
		{
			...
			window.location.href="search.action?m=fronttoload&searchWord=<%=request.getAttribute("searchWord")%>&page=1&pageSize="+pageSize;
		}
	</script>
    ...
    <td width="13"><img src="${skinPath}images/leftmenu-bg03.jpg" width="13" height="35" /></td>
     ...
     <table width="960" border="0" cellpadding="0" cellspacing="0" style="border:solid; border-color:#d0ced1; border-width:0px 1px 1px 1px">
        ...
        <tr>
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td height="30" bgcolor="edf7fd" align="left">&nbsp;&nbsp;&nbsp;&nbsp;显示
                 ...
                 <td  height="30" align="center" bgcolor="edf7fd" class="hei14">找到<span class="lanlink">${totalnum}</span>条数据，共<span class="lanlink">${pageable.maxPages}页</span>&nbsp;&nbsp;
                    <c:choose> 
                        <c:when test="${pageable.page==1}"> 
                            <img src="${skinPath}images/lucene/first-hui.gif" align="absmiddle" />
                            <img src="${skinPath}images/lucene/front-hui.gif" align="absmiddle" />
             ...
             <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style= "table-layout:fixed;">
              <tr  onmousemove="javascript:changebg(this,0)" style="backgroud-color:#f4f4f4" onmouseout="javascript:changebg(this,1)">
                <td  height="40" align="center" background="${skinPath}images/lucene/bg_repeat.png" bgcolor="#4E85E2" class="hei14">序号</td>
	    		...
	    		String Journal=doctxt.getJournal();
	    		String Publication=doctxt.getPublicationType();
	    		String PDATA=doctxt.getPublicationDate();
                %>
                 <tr  onmousemove="javascript:changebg(this,0)" style="backgroud-color:#f4f4f4" onmouseout="javascript:changebg(this,1)">
                ...
                <td  style= "white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"  height="30" align="center" class="hei14"><%=Publication %>&nbsp;&nbsp;&nbsp;</td>
