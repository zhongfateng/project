...
<script type="text/javascript">
	$(function(){
		...
		$("#select").change(function(){
			...
			window.location.href="protein.action?m=frontAdProteinList&NCBISpeciesID=<%=request.getAttribute("NCBISpeciesID")%>&page=1&pageSize="+pageSize+"&chromosomeRef=<%=request.getAttribute("chromosomeRef")%>&ProteinID=<%=request.getAttribute("ProteinID")%>&NcbiProteinRef=<%=request.getAttribute("NcbiProteinRef")%>";
		});
          ...
          <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
              ...
              <tr>
                ...
                &nbsp;&nbsp;第&nbsp;<input name="pageSize" style="width: 50px;" id="proteinPage" value="${page}"/>&nbsp;页&nbsp; 
                <c:if test="${dataList.allRow==0}">
                <input type="button" align="absmiddle" style="height:20px;width:50px;background-image: url('<%=request.getContextPath()%>/resources/images/geneButton/go1.gif');border:0;"/>
                </c:if>
                <c:if test="${dataList.allRow!=0}">
                <input type="button" align="absmiddle" style="height:20px;width:50px;background-image: url('<%=request.getContextPath()%>/resources/images/geneButton/go.gif');border:0;" onclick="protein();"/>
                </c:if>
                </td>
              </tr>
              <tr>
...
<script type="text/javascript">
	function changepage(pid){
	    ...
	    if(pid=='1'){
	    	window.location.href="protein.action?m=frontAdProteinList&page=1&NCBISpeciesID=<%=request.getAttribute("NCBISpeciesID")%>&pageSize="+pageSize+"&chromosomeRef=<%=request.getAttribute("chromosomeRef")%>&ProteinID=<%=request.getAttribute("ProteinID")%>&NcbiProteinRef=<%=request.getAttribute("NcbiProteinRef")%>";
	    }
	    if(pid=='2'){
	    	window.location="protein.action?m=frontAdProteinList&page=${dataList.currentPage-1}&NCBISpeciesID=<%=request.getAttribute("NCBISpeciesID")%>&pageSize="+pageSize+"&chromosomeRef=<%=request.getAttribute("chromosomeRef")%>&ProteinID=<%=request.getAttribute("ProteinID")%>&NcbiProteinRef=<%=request.getAttribute("NcbiProteinRef")%>";
	    }
	    if(pid=='3'){
	    	window.location="protein.action?m=frontAdProteinList&page=${dataList.currentPage+1}&NCBISpeciesID=<%=request.getAttribute("NCBISpeciesID")%>&pageSize="+pageSize+"&chromosomeRef=<%=request.getAttribute("chromosomeRef")%>&ProteinID=<%=request.getAttribute("ProteinID")%>&NcbiProteinRef=<%=request.getAttribute("NcbiProteinRef")%>";
	    }
	    if(pid=='4'){
	   		window.location="protein.action?m=frontAdProteinList&page=${dataList.totalPage}&NCBISpeciesID=<%=request.getAttribute("NCBISpeciesID")%>&pageSize="+pageSize+"&chromosomeRef=<%=request.getAttribute("chromosomeRef")%>&ProteinID=<%=request.getAttribute("ProteinID")%>&NcbiProteinRef=<%=request.getAttribute("NcbiProteinRef")%>";
	    }
	}
</script>
</body>
