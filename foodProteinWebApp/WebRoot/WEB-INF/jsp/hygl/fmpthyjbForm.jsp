...
function checkTheForm(){
   var from=document.myForm;
	if(from.jbmc.value.length==0){
	  	alert("必须填写级别名称！");
	}else if(from.jbjf.value.length==0){
	   alert("必须填写级别积分！");
	}else if(from.jbzk.value.length==0){
	   alert("必须填写级别折扣！");
	}else{
	
	    if( $("#ikey").attr("checked")=='checked'){
            $("#kval").val("1");
	    }
...
function backPage(){
	document.all.frm1.action = "fmpthyjb.action?m=loadData";
	...
	<body>
		<form action="fmpthyjb.action?m=save" method="post" name="myForm">
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0"
				...
				<tr class="tr_nbw">
					...
					<td class="td_nbw_right">
						<input type="text" name="jbjf" id="jbjf" value="${bzpthyjb.jbjf}"
							size="40" />
					</td>
				</tr>
				<tr class="tr_nbw">
					...
					<td class="td_nbw_right">
						<input type="text" name="jbzk" id="jbzk" value="${bzpthyjb.jbzk}"
							size="40" />
					</td>
				</tr>
				<tr class="tr_nbw">
