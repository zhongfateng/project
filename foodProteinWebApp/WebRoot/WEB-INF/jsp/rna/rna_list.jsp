...
var bgcolor;
if(cl=="0"){bgcolor="#FFFFDF";}else{bgcolor="#f1fee9";}
...
<script type="text/javascript">
	$(function(){
		...
		$("#select").change(function(){
			...
			window.location.href="rna.action?m=frontRnaList&rnaInfo=<%=request.getAttribute("INFO")%>&page=1&pageSize="+pageSize;
		});
	...
	function rna(){
		...
		if(page=="" || isNaN(page) || !reg.test(page) || all<page){
			...
			window.location.href="rna.action?m=frontRnaList&rnaInfo=<%=request.getAttribute("INFO")%>&page="+page+"&pageSize="+pageSize;
		}
		
	};
	function rna1(){
		var page = $("#rnaPage1").val();
		var reg = /^[1-9][0-9]*$/;
		var pageSize = <%=request.getAttribute("pageSize")%>;
		var all = $("#yeshu").html();
		if(page=="" || isNaN(page) || !reg.test(page) || all<page){
...
<table width="965" height="692" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="734" height="692" valign="top"><table width="734" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td height="35" background="<%=request.getContextPath()%>/resources/images/rna/images/leftmenu-bg-rna.jpg" class="green14B">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;检索结果列表</td>
     </tr>
    </table>
          ...
          <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" id="dd_00">
                ...
                <tr>
                  ...
                  &nbsp;&nbsp;第&nbsp;<input name="pageSize" style="width: 50px;" id="rnaPage1" value="${page}"/>&nbsp;页&nbsp;
                  ...
                  <c:if test="${dataList.allRow!=0}">
                  <input type="button" align="absmiddle" style="height:20px;width:50px;background-image: url('<%=request.getContextPath()%>/resources/images/geneButton/go.gif');border:0;" onclick="rna1();"/>
                  </c:if>
                  </td>
                </tr>
                <tr>
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0"   style=" margin-bottom:10px;">
                <tr  onmousemove="javascript:changebg(this,0)" style="background:#f1fee9" onmouseout="javascript:changebg(this,1)" >
                  <td  class="hei14" style="padding:5px 40px; border-bottom:solid 1px #cccccc;border-top:solid 1px #cccccc; ">
				  <p>${i.count+dataList.pageSize*(dataList.currentPage-1)}. <a href="rna.action?m=frontRnaDetail&rnaId=${gg.geneId}" class="green14B"><strong>${gg.gid}</strong></a>&nbsp;&nbsp;&nbsp;[${gg.organism}]<br />
                  Chromosome ref: ${gg.chromosomeRef}<br />
               ...
               <c:choose> 
                  ...
                  第${dataList.currentPage}页
                    <c:choose>
               		...
               		&nbsp;&nbsp;第&nbsp;<input name="pageSize" style="width: 50px;" id="rnaPage" value="${page}"/>&nbsp;页&nbsp;
               		<c:if test="${dataList.allRow==0}">
               		<input type="button" align="absmiddle" style="height:20px;width:50px;background-image: url('<%=request.getContextPath()%>/resources/images/geneButton/go1.gif');border:0;"/>
                	</c:if>
                	<c:if test="${dataList.allRow!=0}">
                	<input type="button" align="absmiddle" style="height:20px;width:50px;background-image: url('<%=request.getContextPath()%>/resources/images/geneButton/go.gif');border:0;" onclick="rna();"/>
                	</c:if>
                </td>
              </tr>
              <tr>
    ...
    <td width="226" valign="top" bgcolor="#FFFFFF" ><table width="100%" cellspacing="0" background="<%=request.getContextPath()%>/resources/images/rna/images/right-bg-rna.jpg" bgcolor="226">
      <tr>
      ...
      <table id="dedte" width="100%" border="0" cellpadding="0" cellspacing="0" style="border:solid; border-width:0px 1px 1px 1px; border-color:#cbcbcb; margin-bottom:5px;table-layout: fixed;">
        <c:forEach items="${itemList}" var="dd" varStatus="i" begin="0">
        <tr >
          <td id="deId_${i.count}" align="left" class="hei14" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis; padding:2px 5px 5px 5px; background:url(<%=request.getContextPath()%>/resources/images/rna/images/jhesh_cbg-rna.jpg); background-position:bottom; background-repeat:repeat-x">
              <img src="<%=request.getContextPath()%>/resources/images/rna/images/arrow_045_small-rna.png" width="16" height="16" />&nbsp;<a href="${dd.serchUrl}" target="_blank">${dd.serchWord}</a><br />
       		...
       		<td id="deId2_${i.count}" width="60" align="left" class="hei14" style="background:url(<%=request.getContextPath()%>/resources/images/rna/images/jhesh_cbg-rna.jpg); background-position:bottom; background-repeat:repeat-x">
          	  &nbsp;&nbsp;${dd.serchDb}<br />
          </td>
        </tr>
        </c:forEach>
...
<script type="text/javascript">
	function changepage(pid){
	    ...
	    if(pid=='1'){
	    	window.location.href="rna.action?m=frontRnaList&page=1&rnaInfo=<%=request.getAttribute("INFO")%>&pageSize="+pageSize;
	    }
	    if(pid=='2'){
	    	window.location="rna.action?m=frontRnaList&page=${dataList.currentPage-1}&rnaInfo=<%=request.getAttribute("INFO")%>&pageSize="+pageSize;
	    }
	    if(pid=='3'){
	    	window.location="rna.action?m=frontRnaList&page=${dataList.currentPage+1}&rnaInfo=<%=request.getAttribute("INFO")%>&pageSize="+pageSize;
	    }
	    if(pid=='4'){
	   		window.location="rna.action?m=frontRnaList&page=${dataList.totalPage}&rnaInfo=<%=request.getAttribute("INFO")%>&pageSize="+pageSize;
	    }
	}
</script>
</body>
