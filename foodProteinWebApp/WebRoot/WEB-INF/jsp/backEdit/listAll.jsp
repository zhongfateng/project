		...
		for(i = 0 ; i < objIds.length; i++){
			if(objIds[i].checked == true){
			   if(confirm('确定要删除该条记录吗?')){
					...
					document.all.frm1.action = "usage.action?m=delete";
...
function moddata(){
	...
	if(objIds!=null){
		for(i = 0 ; i < objIds.length; i++){
			...
			if(objIds[i].checked == true){
				document.all.frm1.objectId.value = objIds[i].value;
				document.all.frm1.action = "usage.action?m=toEditPage";
				document.all.frm1.submit();
				break;
			}
		}
...
<table cellpadding="0" cellspacing="0" width="100%" border="0" align="center">
					     ...
					     <tr  style="height: 21">
							<td>
							   <img src="${skinPath}images/button/button_1/zj.gif"
															style="cursor: pointer" onclick="javascript:adddata('usage.action?m=toAddPage');">
	...
	<tr>
		...
		<div style="overFlow: auto; height: 550px">
						<ec:table items="dataList" action="usage.action?m=loadData"
