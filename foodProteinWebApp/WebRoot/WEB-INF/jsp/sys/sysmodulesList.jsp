...
<div id="pageTable">
	<div id="layoutGird2" class="sys_layout" style="float: inherit;height: 155px;"></div>
...
<script>
	...
	function delData(){
		...
		if (null!=ids) {
			function confirmDel(i){if('yes' != i) return false;
				$.ajax({
					type: "POST",
					url:"<%=request.getContextPath()%>/sysmodules.action?m=delete&ids="+ids,
					async: false,
    ...
    dhxLayout.cells("b").setHeight("210");
    dhxLayout.cells("a").setText("模块树");
    dhxLayout.cells("b").setText("模块列表");
    dhxLayout.cells("c").setText("模块详细信息");
	...
	dhxTabbar.addTab("a2", "功能注册", "100px");
	dhxTabbar.cells("a1").attachURL("<%=request.getContextPath()%>/sysmodules.action?m=toFormPage");
	dhxTabbar.setTabActive("a1");
	
	dhxTabbar.cells("a2").attachObject("grididiv");
	dhxGridI = new dhtmlXGridObject("grididiv");
	dhxGridI.setImagePath("<%=request.getContextPath()%>/resources/component/dhtmlxGrid/imgs/");
	dhxGridI.setHeader("modelId,操作码,操作名称,操作方法");
	dhxGridI.setInitWidths("0,180,180,180");
	dhxGridI.setInitWidthsP("0,20,40,40");
	dhxGridI.setColAlign("left,left,left,left");
	dhxGridI.setColTypes("ed,ed,ed,ed");
	dhxGridI.setColSorting("str,str,str,str");
	dhxGridI.selMultiRows = true;
	dhxGridI.init();
	dhxGridI.setSkin("<%=gridSkin %>");
	dhxGridI.loadXML("sysmodules.action?m=loadActionGridXML", function(){});
	dhxGridI.attachEvent("onEditCell", function(stage,rId,cInd,nValue,oValue){
		rowNum = dhxGridI.getRowIndex(rId)+1;
		if (stage == 2){
			switch (cInd) {
				case 1:
					return isNotOverlength(nValue, rowNum, cInd-1, 0, 32);
					break;
				case 2:
					return isNotOverlength(nValue, rowNum, cInd-1, 0, 32);
					break;
	...
	dhxMenu.setIconsPath("<%=request.getContextPath()%>/resources/component/dhtmlxMenu/imgs/");
	dhxMenu.renderAsContextMenu();
	dhxMenu.addNewChild(null, 1, "add", "添加", false, "new.gif");
	dhxMenu.addNewChild(null, 2, "del", "删除", false, "close.gif");
	dhxMenu.addContextZone("grididiv");
	
	dhxTabbar.attachEvent("onSelect", function(id,last_id){
		var moduleId = dhxGrid.getSelectedRowId();
		if("a2"==id){
			dhxGridI.clearAll();
			dhxGridI.loadXML("sysmodules.action?m=loadActionGridXML&moduleId="+moduleId, function(){});
		}
		return true;
	});
	
	dhxMenu.attachEvent("onClick", function(id, zoneId, casState){
		var tabId = dhxTabbar.getActiveTab();
		var moduleId = dhxGrid.getSelectedRowId();
		if ("add"==id){
			if("a2"==tabId){
				var values=[moduleId,"","",""];
				var id = new UUID();
				dhxGridI.addRow(id,values);
				dhxGridI.selectRowById(id);
			}
		} else if ("del"==id){
			if("a2"==tabId){
				dhxGridI.deleteSelectedItem();
			}
		} 
		return true;
	});
	function notEmpty(value, id, ind) {
		if (value == "")
			dhxGridI.setCellTextStyle(id, ind-1, "background-color:yellow;");
		return isNotEmpty(value, dhxGridI.getRowIndex(id)+1, ind);
	}
	myDataProcessor = new dataProcessor("sysmodules.action?m=updateAction");
	myDataProcessor.setUpdateMode("off");
	myDataProcessor.setTransactionMode("POST",false);

	myDataProcessor.setVerificator(1, notEmpty);
	myDataProcessor.init(dhxGridI);
	...
	dhxGrid.setHeader("模块编码,模块名称,模块说明,备注,排序号");
	dhxGrid.attachHeader("#text_filter,#text_filter,#text_filter,#text_filter,&nbsp;");
	dhxGrid.setInitWidthsP("20,20,20,20,20");
	...
	dhxGrid.loadXML("sysmodules.action?m=loadGridXML", function(){
	});
    ...
    function doTreeOnClick(nodeId){
		...
		dhxGrid.clearAll();
		dhxGrid.loadXML("sysmodules.action?m=loadGridXML&id="+nodeId, function(){
			if(addgriid != ""){
    ...
    function doOnRowSelected(id){
		...
		$.ajax({
			type: "POST",
			url:"<%=request.getContextPath()%>/sysmodules.action?m=getFormData&id="+id,
			async: true,
			success:function(data){
