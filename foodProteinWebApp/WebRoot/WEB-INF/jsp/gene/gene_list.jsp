...
<script type="text/javascript">
	$(function(){
		...
		$("#select").change(function(){
			...
			window.location.href="gene.action?m=frontGeneList&geneInfo=<%=request.getAttribute("INFO")%>&page=1&pageSize="+pageSize;
		});
	...
	function gene1(){
		...
		var pageSize = <%=request.getAttribute("pageSize")%>;
		var all = $("#yeshu").html();
		if(page=="" || isNaN(page) || !regg.test(page) || all<page){
			ymPrompt.alert('请输入正确页码！');
		}else {
			window.location.href="gene.action?m=frontGeneList&geneInfo=<%=request.getAttribute("INFO")%>&page="+page+"&pageSize="+pageSize;
		}
...
<table width="965" height="692" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <c:forEach items="${dataList.list}" var="gg" begin="0" varStatus="i">
                ...
                <tr  onmousemove="javascript:changebg(this,0)" style="background:#f4f4f4" onmouseout="javascript:changebg(this,1)" >
                  <td id="d_00" class="hei14" style="padding:5px 40px; border-bottom:solid 1px #cccccc;border-top:solid 1px #cccccc; ">
    ...
    <table width="100%" border="0" cellspacing="0" background="<%=request.getContextPath()%>/resources/images/gene/images/right-bg.jpg" bgcolor="226">
      <tr>
...
<script type="text/javascript">
	function changepage(pid){
	    ...
	    if(pid=='1'){
	    	window.location.href="gene.action?m=frontGeneList&page=1&geneInfo=<%=request.getAttribute("INFO")%>&pageSize="+pageSize;
	    }
	    if(pid=='2'){
	    	window.location="gene.action?m=frontGeneList&page=${dataList.currentPage-1}&geneInfo=<%=request.getAttribute("INFO")%>&pageSize="+pageSize;
	    }
	    if(pid=='3'){
	    	window.location="gene.action?m=frontGeneList&page=${dataList.currentPage+1}&geneInfo=<%=request.getAttribute("INFO")%>&pageSize="+pageSize;
	    }
	    if(pid=='4'){
	   		window.location="gene.action?m=frontGeneList&page=${dataList.totalPage}&geneInfo=<%=request.getAttribute("INFO")%>&pageSize="+pageSize;
	    }
	}
</script>
</body>
