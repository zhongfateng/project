...
var bgcolor;
if(cl=="0"){bgcolor="#FFFFDF";}else{bgcolor="#f4f4f4";}
...
<script type="text/javascript">
	$(function(){
		...
		$("#select").change(function(){
			...
			window.location.href="gene.action?m=frontAdGeneList&NCBISpeciesID=<%=request.getAttribute("NCBISpeciesID")%>&page=1&pageSize="+pageSize+"&chromosomeRef=<%=request.getAttribute("chromosomeRef")%>&genID=<%=request.getAttribute("genID")%>&geneName=<%=request.getAttribute("geneName")%>";
		});
	...
	function gene(){
		...
		if(page=="" || isNaN(page) || !reg.test(page) || all<page){
			...
			window.location.href="gene.action?m=frontGeneList&geneInfo=<%=request.getAttribute("INFO")%>&page="+page+"&pageSize="+pageSize;
		}
...
<table width="965" height="692" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="734" height="692" valign="top"><table width="734" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td height="35" background="<%=request.getContextPath()%>/resources/images/gene/images/leftmenu-bg.jpg" class="hei16B">&nbsp;&nbsp;&nbsp;&nbsp;检索结果列表</td>
     </tr>
    </table>
      <table width="734" border="0" cellpadding="0" cellspacing="0" style="border:solid; border-color:#d0ced1; border-width:0px 1px 1px 1px">
        <tr>
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <table id="dd_00" width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td>&nbsp;</td>
              ...
              <c:forEach items="${dataList.list}" var="gg" begin="0" varStatus="i">
                ...
                <tr  onmousemove="javascript:changebg(this,0)" style="background:#f4f4f4" onmouseout="javascript:changebg(this,1)" >
                  <td  class="hei14" style="padding:5px 40px; border-bottom:solid 1px #cccccc;border-top:solid 1px #cccccc; ">
				  <p>${i.count+dataList.pageSize*(dataList.currentPage-1)}. <a href="gene.action?m=frontGeneDetail&ncbiGeneRef=${gg.ncbiGeneRef}"><strong>${gg.gid}</strong></a>&nbsp;&nbsp;&nbsp;[${gg.organism}]<br />
                  Gene Name: ${gg.gname}<br />
                  Chromosome start: ${gg.chromosomeStart} &nbsp;&nbsp;&nbsp;Chromosome end:${gg.chromosomeEnd}</p></td>
                </tr>
            	</table>
              </c:forEach>
            ...
            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  &nbsp;&nbsp;第&nbsp;<input name="pageSize" style="width: 50px;" id="genePage1" value="${page}"/>&nbsp;页&nbsp; <input type="button" align="absmiddle" style="height:20px;width:50px;background-image: url('<%=request.getContextPath()%>/resources/images/geneButton/go.gif');border:0;" onclick="gene1();"/>
                  </td>
                </tr>
                <tr>
                  <td>&nbsp;</td>
    ...
    <td width="226" valign="top" bgcolor="#FFFFFF" >
    <table width="100%" cellspacing="0" background="<%=request.getContextPath()%>/resources/images/gene/images/right-bg.jpg" bgcolor="226">
      <tr>
      ...
      <table id="dedte" width="100%" border="0" cellpadding="0" cellspacing="0" style="table-layout:fixed;border:solid; border-width:0px 1px 1px 1px; border-color:#cbcbcb; margin-bottom:5px; padding-left:">
        ...
        <tr>
          <td id="deId" align="left" class="hei14" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis; padding:2px 5px 5px 5px; background:url(images/jhesh_cbg.gif); background-position:bottom; background-repeat:repeat-x">
          	  <img src="<%=request.getContextPath()%>/resources/images/gene/images/arrow_045_small.png" width="16" height="16" />&nbsp;<a href="${dd.serchUrl}" target="_blank">${dd.serchWord}</a><br />
          </td>
          <td id="deId2" width="60" align="left" class="hei14" style="background:url(images/jhesh_cbg.gif); background-position:bottom; background-repeat:repeat-x">
          	  &nbsp;&nbsp;${dd.serchDb}<br />
          </td>
        </tr>
        </c:forEach>
      ...
      <table width="100%" cellspacing="0">
        <tr>
          <td height="30" style="padding-left: 20px;" background="<%=request.getContextPath()%>/resources/images/gene/images/right-bg.jpg"><span class="bai14">相关数据库检索 </span></td>
        </tr>
      </table>
      <table width="100%" cellspacing="0" border="0" style="border:solid; border-width:0px 1px 1px 1px; border-color:#cbcbcb;margin-bottom:5px;">
        <tr>
          <td height="20" bgcolor="#F6FCFC" class="hei14" style="padding:2px 15px; background:url(<%=request.getContextPath()%>/resources/images/gene/images/jhesh_cbg.gif); background-position:bottom; background-repeat:repeat-x"><p>
        ...
        <tr>
          <td height="35" bgcolor="#F6FCFC" class="hei14" style="padding:10px 15px; background:url(<%=request.getContextPath()%>/resources/images/gene/images/jhesh_cbg.gif); background-position:bottom; background-repeat:repeat-x"><p>
              ...
              <p align="center"><a href="javascript:;" id="d1_1"><img src="<%=request.getContextPath()%>/resources/images/gene/images/search-menu.jpg" width="102" height="35" border="0" /></a></p>
          </td>
        </tr>
      </table>
      </td>
...
<script type="text/javascript">
	function changepage(pid){
	    ...
	    if(pid=='1'){
	    	window.location.href="gene.action?m=frontAdGeneList&page=1&NCBISpeciesID=<%=request.getAttribute("NCBISpeciesID")%>&pageSize="+pageSize+"&chromosomeRef=<%=request.getAttribute("chromosomeRef")%>&genID=<%=request.getAttribute("genID")%>&geneName=<%=request.getAttribute("geneName")%>";
	    }
	    if(pid=='2'){
	    	window.location="gene.action?m=frontAdGeneList&page=${dataList.currentPage-1}&NCBISpeciesID=<%=request.getAttribute("NCBISpeciesID")%>&pageSize="+pageSize+"&chromosomeRef=<%=request.getAttribute("chromosomeRef")%>&genID=<%=request.getAttribute("genID")%>&geneName=<%=request.getAttribute("geneName")%>";
	    }
	    if(pid=='3'){
	    	window.location="gene.action?m=frontAdGeneList&page=${dataList.currentPage+1}&NCBISpeciesID=<%=request.getAttribute("NCBISpeciesID")%>&pageSize="+pageSize+"&chromosomeRef=<%=request.getAttribute("chromosomeRef")%>&genID=<%=request.getAttribute("genID")%>&geneName=<%=request.getAttribute("geneName")%>";
	    }
	    if(pid=='4'){
	   		window.location="gene.action?m=frontAdGeneList&page=${dataList.totalPage}&NCBISpeciesID=<%=request.getAttribute("NCBISpeciesID")%>&pageSize="+pageSize+"&chromosomeRef=<%=request.getAttribute("chromosomeRef")%>&genID=<%=request.getAttribute("genID")%>&geneName=<%=request.getAttribute("geneName")%>";
	    }
	}
</script>
</body>
