...
<html>
  <head>
	...
	var sql="import.action?m=toJLXXList";
	var rewcods=$("#rewcodsid").val();
	var rewcomname=$("#rewcomnameid").val();	
	if(rewcods!=null&&!rewcods=='')
	{
	sql=sql+"&rewcods="+encodeURI(encodeURI(rewcods));

	}
	if(rewcomname!=null&&!rewcomname=='')
	{
	sql=sql+"&rewcomname="+encodeURI(encodeURI(rewcomname));

	}
	if(sql=="import.action?m=toJLXXList")
	{
	...
	function  resetall()
	{
	document.getElementById('rewcodsid').value='';
	document.getElementById('rewcomnameid').value='';
	}
	...
	var rewcods="${rewcods}";
	rewcods=encodeURI(encodeURI(rewcods));
	
	var rewcomname="${rewcomname}";
	rewcomname=encodeURI(encodeURI(rewcomname));
	    if(pid=='1'){
	    	window.location = "import.action?m=toJLXXList&page=1&pageSize=${jlxxList.pageSize}&rewcods="+rewcods+"&rewcomname="+rewcomname;
	    }
	    if(pid=='2'){
	    	window.location="import.action?m=toJLXXList&page=${jlxxList.currentPage-1}&pageSize=${jlxxList.pageSize}&rewcods="+rewcods+"&rewcomname="+rewcomname;
	    }
	    if(pid=='3'){
	    	window.location="import.action?m=toJLXXList&page=${jlxxList.currentPage+1}&pageSize=${jlxxList.pageSize}&rewcods="+rewcods+"&rewcomname="+rewcomname;
	    }
	    if(pid=='4'){
	   		window.location="import.action?m=toJLXXList&page=${jlxxList.totalPage}&pageSize=${jlxxList.pageSize}&rewcods="+rewcods+"&rewcomname="+rewcomname;
	    }
	}
</script>
	<script type="text/javascript">
		...
		for(i = 0 ; i < objIds.length; i++){
			if(objIds[i].checked == true){
			   if(confirm('确定要删除该条记录吗?')){
					...
					document.all.frm1.action = "import.action?m=deleteJLXXByID";
	...
	function jump()
	{
		...
		var page = $("#pageid").val();
		var reg = /^[1-9][0-9]*$/;
		if(page=="" || isNaN(page) || page<=0 || page>${jlxxList.totalPage}){
			ymPrompt.alert('请输入正确页码！');
		}else{
			window.location.href="import.action?m=toJLXXList&pageSize=${jlxxList.pageSize}&page="+page+"&rewcods="+rewcods+"&rewcomname="+rewcomname;
		}
	}
	</script>
  </head>
        ...
        <td valign="middle"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr style="border-bottom-width:2px; border-bottom-color:#fff; border-bottom-style:solid;">
            ...
            <td height="35" align="right" class="left_txt">组&nbsp; 织&nbsp; 机 &nbsp;构 &nbsp;代 &nbsp;码&nbsp; &nbsp;：</td>  
                <td align="left"><label>
                  <input id="rewcodsid" name="textfield" type="text" class="input4" value="${rewcods }" size="30">
                </label>
              </td> 
                </tr>
              <tr>
              ...
              <td align="left"> <input id="rewcomnameid" name="textfield2" type="text" class="input4" value="${rewcomname }" size="30">
              </td></tr><tr>
    ...
    <option>请选择省……</option>
          ...
          <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
                <tr>
                  ...
                  <td align="center" background="<%=request.getContextPath()%>/resources/images/back/images/a2.gif" class="txt_bt">奖励类别</td>
                  <td align="center" background="<%=request.getContextPath()%>/resources/images/back/images/a2.gif" class="txt_bt">发证日期</td>
                  ...
                  <td align="center" class="txt_bt"  style="background-image:url(<%=request.getContextPath()%>/resources/images/back/images/a3.gif); background-position:right;">产品类别</td>
                </tr>
                <c:forEach items="${jlxxList.list}" var="info" begin="0" varStatus="i">
                <tr>
                  ...
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.rewcods}  </td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.rewcomname } </td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.rewclass }</td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.rewtime }</td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.rewendtime }</td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.reworg} </td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.rewnum }</td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.rewsort }</td>
                </tr>
              </c:forEach>
          ...
          <tr>
            <td align="right">
            <c:choose> 
             <c:when test="${jlxxList.currentPage==1}"> 
            ...
            <td align="left">
               <c:choose>
                        <c:when test="${jlxxList.currentPage < jlxxList.totalPage}">
