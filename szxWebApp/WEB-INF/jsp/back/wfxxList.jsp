...
<html>
  <head>
	...
	var sql="import.action?m=toWFXXList";
	var illcodes=$("#illcodesid").val();
	var illcomname=$("#illcomnameid").val();	
	if(illcodes!=null&&!illcodes=='')
	{
	sql=sql+"&illcodes="+encodeURI(encodeURI(illcodes));

	}
	if(illcomname!=null&&!illcomname=='')
	{
	sql=sql+"&illcomname="+encodeURI(encodeURI(illcomname));

	}
	if(sql=="import.action?m=toWFXXList")
	{
	...
	function  resetall()
	{
	document.getElementById('illcodesid').value='';
	document.getElementById('illcomnameid').value='';
	}
	...
	var illcodes="${illcodes}";
	illcodes=encodeURI(encodeURI(illcodes));
	
	var illcomname="${illcomname}";
	illcomname=encodeURI(encodeURI(illcomname));
	    if(pid=='1'){
	    	window.location = "import.action?m=toWFXXList&page=1&pageSize=${wfxxList.pageSize}&illcodes="+illcodes+"&illcomname="+illcomname;
	    }
	    if(pid=='2'){
	    	window.location="import.action?m=toWFXXList&page=${wfxxList.currentPage-1}&pageSize=${wfxxList.pageSize}&illcodes="+illcodes+"&illcomname="+illcomname;
	    }
	    if(pid=='3'){
	    	window.location="import.action?m=toWFXXList&page=${wfxxList.currentPage+1}&pageSize=${wfxxList.pageSize}&illcodes="+illcodes+"&illcomname="+illcomname;
	    }
	    if(pid=='4'){
	   		window.location="import.action?m=toWFXXList&page=${wfxxList.totalPage}&pageSize=${wfxxList.pageSize}&illcodes="+illcodes+"&illcomname="+illcomname;
	    }
	}
</script>
<script type="text/javascript">
	function jump()
	{
		...
		var page = $("#pageid").val();
		var reg = /^[1-9][0-9]*$/;
		if(page=="" || isNaN(page) || page<=0 || page>${wfxxList.totalPage}){
			ymPrompt.alert('请输入正确页码！');
		}else{
			window.location.href="import.action?m=toWFXXList&pageSize=${wfxxList.pageSize}&page="+page+"&illcodes="+illcodes+"&illcomname="+illcomname;
		}
	}
	</script>
<script type="text/javascript">
		...
		for(i = 0 ; i < objIds.length; i++){
			if(objIds[i].checked == true){
			   if(confirm('确定要删除该条记录吗?')){
					...
					document.all.frm1.action = "import.action?m=deleteWFXXByID";
  ...
  <tr>
    ...
    <td width="144" align="center" background="<%=request.getContextPath()%>/resources/images/back/images/man-info_13.png" class="txt_bt"><span class="login_txt_bt">企业奖励信息</span></td>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr style="border-bottom-width:2px; border-bottom-color:#fff; border-bottom-style:solid;">
            ...
            <td height="35" align="right" class="left_txt">组&nbsp; 织&nbsp; 机 &nbsp;构 &nbsp;代 &nbsp;码&nbsp; &nbsp;：</td>  
                <td align="left"><label>
                  <input id="illcodesid" name="textfield" type="text" class="input4" value="${illcodes }" size="30">
                </label>
              </td> 
                </tr>
              <tr>
              ...
              <td align="left"> <input id="illcomnameid" name="textfield2" type="text" class="input4" value="${illcomname }" size="30">
              </td></tr><tr>
    ...
    <option>请选择省……</option>
          ...
          <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC" style= "table-layout:fixed;">
                <tr>
                  ...
                  <td align="center" background="<%=request.getContextPath()%>/resources/images/back/images/a2.gif" class="txt_bt">违规类型</td>
                  <td align="center" background="<%=request.getContextPath()%>/resources/images/back/images/a2.gif" class="txt_bt">违法内容</td>
                  <td align="center" class="txt_bt"  style="background-image:url(<%=request.getContextPath()%>/resources/images/back/images/a3.gif); background-position:right;">处理结果</td>
                  <td align="center" class="txt_bt"  style="background-image:url(<%=request.getContextPath()%>/resources/images/back/images/a3.gif); background-position:right;">违法日期</td>
                  <td align="center" class="txt_bt"  style="background-image:url(<%=request.getContextPath()%>/resources/images/back/images/a3.gif); background-position:right;">处罚单位</td>
                  <td align="center" class="txt_bt"  style="background-image:url(<%=request.getContextPath()%>/resources/images/back/images/a3.gif); background-position:right;">行政处罚类型</td>
                </tr>
                <c:forEach items="${wfxxList.list}" var="info" begin="0" varStatus="i">
                <tr>
                  ...
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.illcodes}  </td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.illcomname } </td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.illclass }</td>
                  <td style= "white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" align="center" bgcolor="#F7F8F9" class="left_txt"><a href="javascript:void(0)" title="${info.illcontent }">${info.illcontent }</a>></td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.illresult }</td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.illtime} </td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.illcompany }</td>
                  <td align="center" bgcolor="#F7F8F9" class="left_txt">${info.illpuniclass }</td>
                </tr>
              </c:forEach>
          ...
          <tr>
            <td align="right">
            <c:choose> 
             <c:when test="${wfxxList.currentPage==1}"> 
            ...
            <td align="center" class="left_txt">第
              ...
              &nbsp;页
			<label>
		<input type="image" src="<%=request.getContextPath()%>/resources/images/back/images/go.gif" onclick="jump()" />
		</label>              
              </td>
            <td align="left">
               <c:choose>
                        <c:when test="${wfxxList.currentPage < wfxxList.totalPage}">
