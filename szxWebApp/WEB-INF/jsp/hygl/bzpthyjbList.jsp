		...
		for(i = 0 ; i < objIds.length; i++){
			if(objIds[i].checked == true){
			   if(confirm('确定要删除该条记录吗?')){
					...
					document.all.frm1.action = "bzpthyjb.action?m=delete";
...
function moddata(){
	...
	if(objIds!=null){
		for(i = 0 ; i < objIds.length; i++){
			if(objIds[i].checked == true){
				document.all.frm1.objectId.value = objIds[i].value;
				document.all.frm1.action = "bzpthyjb.action?m=toEditPage";
				document.all.frm1.submit();
				break;
			}
		}
	...
	<body style="margin: 0px;">
		<table cellpadding="0" cellspacing="0" width="100%" border="0"
												...
												<tr style="height: 21">
													<td>
														<img src="${skinPath}images/button/button_1/zj.gif"
															...
															onclick="javascript:adddata('bzpthyjb.action?m=toAddPage');">
													</td>
													<td style="width: 3">
														&nbsp;
													</td>
			...
			<tr>
				<td>
					<ec:table items="dataList" action="bzpthyjb.action?m=loadData"
