...
(function (b) {
	b.jgrid = b.jgrid || {};
	b.extend(b.jgrid, {htmlDecode:function (e) {
		...
		var i = /<("[^"]*"|'[^']*'|[^'">])*>/gi;
		if (e) {
			return (e = e.replace(i, "")) && e !== "&nbsp;" && e !== "&#160;" ? e.replace(/\"/g, "'") : "";
		} else {
			return e;
		}
		...
		e || (e = "{}");
		return b.jgrid.useJSON === true && typeof JSON === "object" && typeof JSON.parse === "function" ? JSON.parse(e) : eval("(" + e + ")");
	}, parseDate:function (e, i) {
		var g = {m:1, d:1, y:1970, h:0, i:0, s:0, u:0}, c, h, k;
		c = /[\\\/:_;.,\t\T\s-]/;
		...
		return new function (i, g) {
			...
			this.startsWith = function (o, n) {
				...
				if (V) {
					c._append(c._getStr("jQuery.jgrid.getAccessor(this,'" + o + "')") + ".substr(0," + t + ") == " + c._getStr("\"" + c._toStr(n) + "\""));
				} else {
		...
		return this.each(function () {
			if (!this.grid) {
				var c = b.extend(true, {url:"", height:150, page:1, rowNum:20, rowTotal:null, records:0, pager:"", pgbuttons:true, pginput:true, colModel:[], rowList:[], colNames:[], sortorder:"asc", sortname:"", datatype:"xml", mtype:"GET", altRows:false, selarrrow:[], savedRow:[], shrinkToFit:true, xmlReader:{}, jsonReader:{}, subGrid:false, subGridModel:[], reccount:0, lastpage:0, lastsort:0, selrow:null, beforeSelectRow:null, onSelectRow:null, onSortCol:null, ondblClickRow:null, onRightClickRow:null, onPaging:null, onSelectAll:null, loadComplete:null, gridComplete:null, loadError:null, loadBeforeSend:null, afterInsertRow:null, beforeRequest:null, beforeProcessing:null, onHeaderClick:null, viewrecords:false, loadonce:false, multiselect:false, multikey:false, editurl:null, search:false, caption:"", hidegrid:true, hiddengrid:false, postData:{}, userData:{}, treeGrid:false, treeGridModel:"nested", treeReader:{}, treeANode:-1, ExpandColumn:null, tree_root_level:0, prmNames:{page:"page", rows:"rows", sort:"sidx", order:"sord", search:"_search", nd:"nd", id:"id", oper:"oper", editoper:"edit", addoper:"add", deloper:"del", subgridid:"id", npage:null, totalrows:"totalrows"}, forceFit:false, gridstate:"visible", cellEdit:false, cellsubmit:"remote", nv:0, loadui:"enable", toolbar:[false, ""], scroll:false, multiboxonly:false, deselectAfterSort:true, scrollrows:false, autowidth:false, scrollOffset:18, cellLayout:5, subGridWidth:20, multiselectWidth:20, gridview:false, rownumWidth:25, rownumbers:false, pagerpos:"center", recordpos:"right", footerrow:false, userDataOnFooter:false, hoverrows:true, altclass:"ui-priority-secondary", viewsortcols:[false, "vertical", true], resizeclass:"", autoencode:false, remapColumns:[], ajaxGridOptions:{}, direction:"ltr", toppager:false, headertitles:false, scrollTimeout:40, data:[], _index:{}, grouping:false, groupingView:{groupField:[], groupOrder:[], groupText:[], groupColumnShow:[], groupSummary:[], showSummaryOnHide:false, sortitems:[], sortnames:[], groupDataSorted:false, summary:[], summaryval:[], plusicon:"ui-icon-circlesmall-plus", minusicon:"ui-icon-circlesmall-minus"}, ignoreCase:false, cmTemplate:{}, idPrefix:""}, b.jgrid.defaults, e || {}), h = {headers:[], cols:[], footers:[], dragStart:function (d, f, j) {
					...
					b.isFunction(c.resizeStart) && c.resizeStart.call(this, f, d);
					...
					if (this.resizing) {
						...
						var j = c.direction === "ltr" ? d.width + f : d.width - f, m;
						if (j > 33) {
							this.curGbox.css({left:this.resizing.sOL + f});
							if (c.forceFit === true) {
					...
					if (this.resizing) {
						var d = this.resizing.idx, f = this.headers[d].newWidth || this.headers[d].width;
						f = parseInt(f, 10);
						this.resizing = false;
						...
						if (c.forceFit === true) {
							f = this.headers[d + c.nv].newWidth || this.headers[d + c.nv].width;
							this.headers[d + c.nv].width = f;
							this.headers[d + c.nv].el.style.width = f + "px";
							this.cols[d + c.nv].style.width = f + "px";
							if (this.footers.length > 0) {
								this.footers[d + c.nv].style.width = f + "px";
							}
						...
						b.isFunction(c.resizeStop) && c.resizeStop.call(this, f, d);
					}
					this.curGbox = null;
					document.onselectstart = function () {
						return true;
					...
					if (d) {
						...
						if (m) {
							...
							m *= C;
							var z, u, w;
							if (A < d && s <= 0 && (c.lastpage === undefined || parseInt((A + j + m - 1) / m, 10) <= c.lastpage)) {
					...
					var f = d.p, j = f.selrow, m = f.selarrrow ? b.makeArray(f.selarrrow) : null, p = d.grid.bDiv.scrollLeft, C = f.gridComplete;
					f.gridComplete = function () {
						f.selrow = null;
						f.selarrrow = [];
						if (f.multiselect && m && m.length > 0) {
							for (var s = 0; s < m.length; s++) {
								m[s] != j && b(d).jqGrid("setSelection", m[s], false);
							}
						}
					...
					if (this.p.colNames.length !== this.p.colModel.length) {
						...
						var M = function (d, f) {
							...
							if (f === 0) {
								...
								if (s.cellattr && b.isFunction(s.cellattr)) {
									if ((d = s.cellattr.call(a, p, j, m, s, C)) && typeof d === "string") {
										...
										if (r.length === 2) {
											...
											if (r[1].indexOf("'") === 0 || r[1].indexOf("\"") === 0) {
												r[1] = r[1].substring(1);
											}
											z += r[1].replace(/'/gi, "\"");
										} else {
							...
							if (typeof C.formatter !== "undefined") {
								d = {rowId:d, colModel:C, gid:a.p.id, pos:j};
								f = b.isFunction(C.formatter) ? C.formatter.call(a, f, d, m, p) : b.fmatter ? b.fn.fmatter(C.formatter, f, d, m, p) : V(f);
							} else {
								f = V(f);
							}
							return f;
							...
							f = o(d, f, j, p, "add");
							return "<td role=\"gridcell\" " + N(j, m, f, p, d, true) + ">" + f + "</td>";
						}, t = function (d, f, j) {
							var m = "<input role=\"checkbox\" type=\"checkbox\" id=\"jqg_" + a.p.id + "_" + d + "\" class=\"cbox\" name=\"jqg_" + a.p.id + "_" + d + "\"/>";
							return "<td role=\"gridcell\" " + N(f, j, "", null, d, true) + ">" + m + "</td>";
						}, F = function (d, f, j, m) {
							j = (parseInt(j, 10) - 1) * parseInt(m, 10) + 1 + f;
							return "<td role=\"gridcell\" class=\"ui-state-default jqgrid-rownum\" " + N(d, f, j, null, f, true) + ">" + j + "</td>";
						}, T = function (d) {
							var f, j = [], m = 0, p;
							for (p = 0; p < a.p.colModel.length; p++) {
								f = a.p.colModel[p];
								if (f.name !== "cb" && f.name !== "subgrid" && f.name !== "rn") {
									j[m] = d == "local" ? f.name : d == "xml" || d === "xmlstring" ? f.xmlmap || f.name : f.jsonmap || f.name;
									m++;
								}
							}
							return j;
							...
							if (b.isXMLDoc(d)) {
								...
								a.p.records = b.jgrid.getXmlData(d, A.records) || 0;
								if (b.isFunction(A.userdata)) {
									a.p.userData = A.userdata.call(a, d) || {};
								} else {
									b.jgrid.getXmlData(d, A.userdata, true).each(function () {
										a.p.userData[this.getAttribute("name")] = b(this).text();
									});
								}
								d = b.jgrid.getXmlData(d, A.root, true);
								(d = b.jgrid.getXmlData(d, A.row, true)) || (d = []);
								var $ = d.length, W = 0, ha = {}, ja = parseInt(a.p.rowNum, 10);
								if ($ > 0 && a.p.page <= 0) {
									a.p.page = 1;
								}
								if (d && $) {
									...
									for (; W < $; ) {
										...
										if (a.p.grouping) {
											...
											for (var Da = [], za = 0; za < u; za++) {
												Da.push(v[a.p.groupingView.groupField[za]]);
											}
								...
								if (a.p.gridview === true) {
									...
									if (a.p.grouping) {
										...
										a.p.treeGrid === true && w > 0 ? b(a.rows[w]).after(U.join("")) : b("tbody:first", f).append(U.join(""));
									}
								}
								if (a.p.subGrid === true) {
									try {
								...
								if (s) {
									a.p.records = $;
									a.p.lastpage = Math.min(a.p.page, Math.ceil($ / ja));
								}
							...
							if (d) {
								...
								a.p.records = b.jgrid.getAccessor(d, f.records) || 0;
								a.p.userData = b.jgrid.getAccessor(d, f.userdata) || {};
								...
								(Z = b.jgrid.getAccessor(d, f.root)) || (Z = []);
								L = Z.length;
								d = 0;
								if (L > 0 && a.p.page <= 0) {
									a.p.page = 1;
								...
								for (; d < L; ) {
									...
									if (a.p.subGrid) {
										H.push(b(a).jqGrid("addSubGridCell", O + K, d + j));
										S = 1;
									}
									if (f.repeatitems) {
										if (f.cell) {
									...
									if (a.p.grouping) {
										...
										for (var qa = 0; qa < u; qa++) {
											w.push(P[a.p.groupingView.groupField[qa]]);
										}
								...
								if (a.p.gridview === true) {
									...
									if (a.p.grouping) {
										...
										a.p.treeGrid === true && v > 0 ? b(a.rows[v]).after(H.join("")) : b("#" + b.jgrid.jqID(a.p.id) + " tbody:first").append(H.join(""));
									}
								}
								if (a.p.subGrid === true) {
									try {
								...
								if (A) {
									a.p.records = L;
									a.p.lastpage = Math.min(a.p.page, Math.ceil(L / ia));
								}
								m || a.updatepager(false, true);
								if (A) {
									for (; z < L && Z[z]; ) {
										...
										if (B === undefined) {
											...
											if (r.length === 0) {
												if (f.cell) {
													B = b.jgrid.getAccessor(p, f.cell)[s] || B;
												}
											}
										}
										if (p) {
							...
							if (b.isArray(a.p.data)) {
								...
								b.each(a.p.colModel, function () {
									...
									if (A == "date" || A == "datetime") {
										...
										m[this.name] = {stype:A, srcfmt:"", newfmt:""};
									}
								...
								if (a.p.treeGrid) {
									b(a).jqGrid("SortTree", f, a.p.sortorder, m[f].stype, m[f].srcfmt);
								} else {
									var w = {eq:function (v) {
										return v.equals;
									}, ne:function (v) {
									...
									if (f && a.p.sortorder && j) {
										a.p.sortorder.toUpperCase() == "DESC" ? r.orderBy(a.p.sortname, "d", m[f].stype, m[f].srcfmt) : r.orderBy(a.p.sortname, "a", m[f].stype, m[f].srcfmt);
									}
									G = r.select();
									var S = parseInt(a.p.rowNum, 10), K = G.length, L = parseInt(a.p.page, 10), Z = Math.ceil(K / S), P = {};
									G = G.slice((L - 1) * S, L * S);
									m = r = null;
							...
							if (!a.grid.hDiv.loading) {
								...
								if (a.p.grouping) {
									...
									if (a.p.groupingView.groupDataSorted === true) {
										j[p.sort] = a.p.groupingView.groupField[0] + " " + a.p.groupingView.groupOrder[0] + ", " + j[p.sort];
									}
								}
								...
								if (b.isFunction(a.p.datatype)) {
									...
									switch (m) {
									  ...
									  case "xml":
									  case "script":
										b.ajax(b.extend({url:a.p.url, type:a.p.mtype, dataType:m, data:b.isFunction(a.p.serializeGridData) ? a.p.serializeGridData.call(a, a.p.postData) : a.p.postData, success:function (u, w, r) {
											b.isFunction(a.p.beforeProcessing) && a.p.beforeProcessing.call(a, u, w, r);
											m === "xml" ? X(u, a.grid.bDiv, z, d > 1, A) : fa(u, a.grid.bDiv, z, d > 1, A);
											C && C.call(a, u);
											...
											var r = true;
											if (b.isFunction(a.p.loadBeforeSend)) {
												r = a.p.loadBeforeSend.call(a, u, w);
											}
											if (r === undefined) {
												r = true;
											}
							...
							b("#cb_" + b.jgrid.jqID(a.p.id), a.grid.hDiv)[a.p.useProp ? "prop" : "attr"]("checked", d);
							if (a.p.frozenColumns && a.p.id + "_frozen") {
								b("#cb_" + b.jgrid.jqID(a.p.id), a.grid.fhDiv)[a.p.useProp ? "prop" : "attr"]("checked", d);
							}
						};
						x = function (d, f) {
							...
							A = d + "_center";
							z = d + "_right";
							b("#" + b.jgrid.jqID(d)).append("<div id='" + C + "' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='" + s + "' align='left'></td><td id='" + A + "' align='center' style='white-space:pre;'></td><td id='" + z + "' align='right'></td></tr></tbody></table></div>").attr("dir", "ltr");
							...
							if (a.p.pginput === true) {
								j = "<td dir='" + l + "'>" + b.jgrid.format(a.p.pgtext || "", "<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/>", "<span id='sp_1_" + b.jgrid.jqID(d) + "'></span>") + "</td>";
							}
							...
							m += "</tr></tbody></table>";
							a.p.viewrecords === true && b("td#" + d + "_" + a.p.recordpos, "#" + C).append("<div dir='" + l + "' style='text-align:" + a.p.recordpos + "' class='ui-paging-info'></div>");
							b("td#" + d + "_" + a.p.pagerpos, "#" + C).append(m);
							...
							if (m > 0) {
								...
								b("td#" + d + "_" + a.p.pagerpos, "#" + C).width(m);
							}
							a.p._nvtd = [];
						...
						var Ea = function (d, f, j, m) {
								...
								if (!(a.p.savedRow.length > 0)) {
									if (!j) {
										if (a.p.lastsort == f) {
											...
											a.p.sortorder = a.p.colModel[f].firstsortorder || "asc";
										}
										a.p.page = 1;
									}
									if (m) {
										if (a.p.lastsort == f && a.p.sortorder == m && !j) {
											return;
										} else {
											a.p.sortorder = m;
										}
							...
							if (b(a.grid.cDiv).is(":visible")) {
								j[1] += b(a.grid.cDiv).height() + parseInt(b(a.grid.cDiv).css("padding-top"), 10) + parseInt(b(a.grid.cDiv).css("padding-bottom"), 10);
							}
							if (a.p.toolbar[0] === true && (a.p.toolbar[1] == "top" || a.p.toolbar[1] == "both")) {
								j[1] += b(a.grid.uDiv).height() + parseInt(b(a.grid.uDiv).css("border-top-width"), 10) + parseInt(b(a.grid.uDiv).css("border-bottom-width"), 10);
							}
							if (a.p.toppager) {
								j[1] += b(a.grid.topDiv).height() + parseInt(b(a.grid.topDiv).css("border-bottom-width"), 10);
							}
							j[2] += b(a.grid.bDiv).height() + b(a.grid.hDiv).height();
							return j;
						}, Fa = function (d) {
							var f, j = a.grid.headers, m = b.jgrid.getCellIndex(d);
						...
						if (this.p.multiselect) {
							...
							b("#cb_" + b.jgrid.jqID(a.p.id), this).bind("click", function () {
								a.p.selarrrow = [];
								var d = a.p.frozenColumns === true ? a.p.id + "_frozen" : "";
								if (this.checked) {
						...
						(function () {
							...
							if (a.p.shrinkToFit === true && j > 0) {
								...
								if (s) {
									...
									if (!s && Math.abs(h.width - z - (d + f * j)) !== 1) {
										w = h.width - z - (d + f * j);
									}
								}
						...
						var wa = "";
						if (a.p.footerrow) {
							wa += "<table role='grid' style='width:" + a.p.tblwidth + "px' class='ui-jqgrid-ftable' cellspacing='0' cellpadding='0' border='0'><tbody><tr role='row' class='ui-widget-content footrow footrow-" + l + "'>";
						}
						q = b("tr:first", da);
						...
						b("th", q).each(function (d) {
							...
							if (a.p.footerrow) {
								wa += "<td role='gridcell' " + N(d, 0, "", null, "", false) + ">&#160;</td>";
							}
						...
						b(a).before(h.hDiv).click(function (d) {
							...
							if (b(Q).length === 0 || Q[0].className.indexOf("ui-state-disabled") > -1 || b(Y, a).closest("table.ui-jqgrid-btable")[0].id.replace("_frozen", "") !== a.id) {
								return this;
							}
							...
							if (j === true) {
								...
								if (b.isFunction(a.p.onCellSelect)) {
									la = Q[0].id;
									ma = b.jgrid.getCellIndex(Y);
									a.p.onCellSelect.call(a, la, ma, b(Y).html(), d);
								}
							}
							return this;
						...
						b.isFunction(this.p.ondblClickRow) && b(this).dblclick(function (d) {
							...
							la = Q[0].rowIndex;
							ma = b.jgrid.getCellIndex(Y);
							a.p.ondblClickRow.call(a, b(Q).attr("id"), la, ma, d);
							return false;
						});
						b.isFunction(this.p.onRightClickRow) && b(this).bind("contextmenu", function (d) {
							...
							la = Q[0].rowIndex;
							ma = b.jgrid.getCellIndex(Y);
							a.p.onRightClickRow.call(a, b(Q).attr("id"), la, ma, d);
							return false;
						});
						...
						b(h.bDiv).append(b("<div style=\"position:relative;" + (y && b.browser.version < 8 ? "height:0.01%;" : "") + "\"></div>").append("<div></div>").append(this)).addClass("ui-jqgrid-bdiv").css({height:a.p.height + (isNaN(a.p.height) ? "" : "px"), width:h.width + "px"}).scroll(h.scrollGrid);
						...
						if (y) {
							...
							a.p.multikey && b(h.bDiv).bind("mousedown", function () {
								return false;
							});
						}
						ea && b(h.bDiv).hide();
						...
						if (a.p.caption) {
							...
							if (a.p.hidegrid === true) {
								b(".ui-jqgrid-titlebar-close", h.cDiv).click(function (d) {
									...
									if (a.p.gridstate == "visible") {
										closeDiv();
										/*b(j, "#gbox_" + b.jgrid.jqID(a.p.id)).slideUp("fast", function () {
											m--;
											if (m === 0) {
										...
										*/
										...
										a.p.gridstate == "hidden" && b(j, "#gbox_" + b.jgrid.jqID(a.p.id)).slideDown("fast", function () {
											m--;
											if (m === 0) {
						...
						a.updatepager = function (d, f) {
							...
							if (u) {
								...
								if (a.p.viewrecords) {
									if (a.p.reccount === 0) {
										b(".ui-paging-info", u).html(a.p.emptyrecords);
									} else {
										C = p + 1;
										A = a.p.records;
										if (b.fmatter) {
							...
							d === true && a.p.rownumbers === true && b("td.jqgrid-rownum", a.rows).each(function (G) {
								b(this).html(p + 1 + G);
							});
	...
	b.jgrid.extend({getGridParam:function (e) {
		var i = this[0];
		if (i && i.grid) {
		...
		return this.each(function () {
			this.grid && typeof e === "object" && b.extend(true, this.p, e);
		});
	}, getDataIDs:function () {
		var e = [], i = 0, g, c = 0;
		this.each(function () {
			...
			if (e !== undefined) {
				...
				if (!(!k || !k.className || k.className.indexOf("ui-state-disabled") > -1)) {
					...
					if (c.p.multiselect) {
						...
						if (q) {
							l === -1 ? b("#" + b.jgrid.jqID(e), "#" + b.jgrid.jqID(q)).addClass("ui-state-highlight") : b("#" + b.jgrid.jqID(e), "#" + b.jgrid.jqID(q)).removeClass("ui-state-highlight");
							b("#jqg_" + b.jgrid.jqID(c.p.id) + "_" + b.jgrid.jqID(e), "#" + b.jgrid.jqID(q))[c.p.useProp ? "prop" : "attr"]("checked", h);
						}
						c.p.onSelectRow && i && c.p.onSelectRow.call(c, k.id, h);
					} else {
						if (k.className !== "ui-subgrid") {
							if (c.p.selrow != k.id) {
								...
								if (q) {
									...
									b("#" + b.jgrid.jqID(e), "#" + b.jgrid.jqID(q)).addClass("ui-state-highlight");
								}
								h = true;
							} else {
								h = false;
				...
				if (b(q).hasClass("jqgrow")) {
					b("td", q).each(function (x) {
						...
						if (a !== "cb" && a !== "subgrid" && a !== "rn") {
							if (l.p.treeGrid === true && a == l.p.ExpandColumn) {
								i[a] = b.jgrid.htmlDecode(b("span:first", this).html());
							} else {
								try {
									i[a] = b.unformat(this, {rowId:q.id, colModel:l.p.colModel[x]}, x);
								}
								catch (y) {
					...
					if (l.p.datatype == "local") {
						...
						if (l.p.treeGrid) {
							for (var I in l.p.treeReader) {
								y.hasOwnProperty(l.p.treeReader[I]) && delete y[l.p.treeReader[I]];
							}
						}
						if (typeof E != "undefined") {
							l.p.data[E] = b.extend(true, l.p.data[E], y);
						}
						y = null;
					}
				}
		...
		if (i) {
			...
			this.each(function () {
				...
				o = this.p.altclass;
				for (var T = 0, J = "", D = {}, ca = b.isFunction(this.p.afterInsertRow) ? true : false; T < F; ) {
					n = i[T];
					l = "";
					if (N) {
						try {
					...
					if (a) {
						I = "<input role=\"checkbox\" type=\"checkbox\" id=\"jqg_" + this.p.id + "_" + e + "\" class=\"cbox\"/>";
					...
					for (E = a + q + x; E < this.p.colModel.length; E++) {
						...
						I = this.formatter(e, b.jgrid.getAccessor(n, k), E, n, "edit");
						M = this.formatCol(E, 1, I, n, e, true);
						l += "<td role=\"gridcell\" aria-describedby=\"" + this.p.id + "_" + k + "\" " + M + ">" + I + "</td>";
					}
					l = "<tr id=\"" + e + "\" role=\"row\" tabindex=\"-1\" class=\"ui-widget-content jqgrow ui-row-" + this.p.direction + " " + J + "\">" + l + "</tr>";
						...
						switch (g) {
						  ...
						  case "after":
							if (y = this.rows.namedItem(c)) {
								b(this.rows[y.rowIndex + 1]).hasClass("ui-subgrid") ? b(this.rows[y.rowIndex + 1]).after(l) : b(y).after(l);
							}
							y++;
							break;
						  case "before":
				...
				if (this.p.altRows === true && !N) {
					if (g == "last") {
						(this.rows.length - 1) % 2 == 1 && b(this.rows[this.rows.length - 1]).addClass(o);
					} else {
		...
		this.each(function () {
			...
			b(this.p.colModel).each(function (y) {
				...
				if (e == "set") {
					...
					if (e == "get") {
						l[h] = b("tr.footrow td:eq(" + y + ")", q.grid.sDiv).html();
					}
				}
			});
		});
			...
			if (g.grid) {
				...
				b(this.p.colModel).each(function (q) {
					if (b.inArray(this.name, e) !== -1 && this.hidden === l) {
						if (g.p.frozenColumns === true && this.frozen === true) {
							return true;
						}
				...
				if (c === true) {
					b(g).jqGrid("setGridWidth", g.p.shrinkToFit === true ? g.p.tblwidth : g.p.width);
				}
		...
		return this.each(function () {
			if (this.grid) {
				var g = this, c, h = 0, k = b.browser.webkit || b.browser.safari ? 0 : g.p.cellLayout, l, a = 0, q = false, x = g.p.scrollOffset, y, E = 0, I = 0, M;
				if (typeof i != "boolean") {
					i = g.p.shrinkToFit;
				}
				if (!isNaN(e)) {
					...
					b("#gbox_" + b.jgrid.jqID(g.p.id)).css("width", e + "px");
					b("#gview_" + b.jgrid.jqID(g.p.id)).css("width", e + "px");
					...
					if (g.p.toolbar[0] === true) {
						...
						g.p.toolbar[1] == "both" && b(g.grid.ubDiv).css("width", e + "px");
					}
					...
					if (i === true) {
						...
						y = e - k * a - E;
						if (!isNaN(g.p.height)) {
							if (b(g.grid.bDiv)[0].clientHeight < b(g.grid.bDiv)[0].scrollHeight || g.rows.length === 1) {
								q = true;
								y -= x;
							}
						}
						...
						b.each(g.p.colModel, function (V) {
							if (this.hidden === false && !this.fixed) {
								...
								if (!(c < 0)) {
									...
									if (g.p.footerrow) {
										g.grid.footers[V].style.width = c + "px";
									}
									if (N) {
										g.grid.cols[V].style.width = c + "px";
									}
									l = V;
								}
							}
		...
		return this.each(function () {
			if (this.grid) {
				b(this.grid.bDiv).css({height:e + (isNaN(e) ? "" : "px")});
				this.p.height = e;
			...
			if (this.grid) {
				if (typeof e != "undefined") {
					...
					if (h >= 0) {
						...
						if (g) {
							typeof g === "string" ? b(k).addClass(g) : b(k).css(g);
						}
		...
		this.each(function () {
			...
			if (this.grid) {
				...
				if (c >= 0) {
					...
					if (h) {
						try {
							g = b.unformat(b("td:eq(" + c + ")", h), {rowId:h.id, colModel:this.p.colModel[c]}, c);
						}
						catch (k) {
					...
					if (y && y > 0) {
						for (; E < y; ) {
							if (b(this.rows[E]).hasClass("jqgrow")) {
								try {
									h = b.unformat(b(this.rows[E].cells[x]), {rowId:this.rows[E].id, colModel:this.p.colModel[x]}, x);
								}
								catch (I) {
								...
								if (g) {
									...
									i ? c.push({id:this.rows[E].id, value:h}) : c.push(h);
								}
							}
							E++;
						}
		...
		var i = b.extend({onEnter:null, onSpace:null, onLeftKey:null, onRightKey:null, scrollingRows:true}, e || {});
		return this.each(function () {
			var g = this;
			...
			b(g).keydown(function (c) {
				...
				if (h) {
					...
					if (c.keyCode === 37 || c.keyCode === 38 || c.keyCode === 39 || c.keyCode === 40) {
						...
						if (c.keyCode === 13) {
							b.isFunction(i.onEnter) && i.onEnter.call(g, g.p.selrow);
						} else {
							c.keyCode === 32 && b.isFunction(i.onSpace) && i.onSpace.call(g, g.p.selrow);
						}
					}
				}
			});
...
(function (b) {
	...
	b.fmatter.util = {NumberFormat:function (a, c) {
		...
		var f = /^\/Date\((([-+])?[0-9]+)(([-+])([0-9]{2})([0-9]{2}))?\)\/$/, g = typeof c === "string" ? c.match(f) : null;
		f = function (m, r) {
			m = String(m);
			...
			if (c.constructor === Date) {
				...
				if (g !== null) {
					...
					if (g[3]) {
						...
						i.setTime(Number(Number(i) + a * 60000));
					}
				} else {
	...
	b.fn.fmatter.select = function (a, c) {
		...
		if (b.fmatter.isUndefined(c.colModel.formatoptions)) {
			if (!b.fmatter.isUndefined(c.colModel.editoptions)) {
				...
				f = c.colModel.editoptions.separator === undefined ? ":" : c.colModel.editoptions.separator;
			}
		} else {
			d = c.colModel.formatoptions.value;
			f = c.colModel.formatoptions.separator === undefined ? ":" : c.colModel.formatoptions.separator;
		}
		if (d) {
	...
	b.fn.fmatter.rowactions = function (a, c, d, e) {
		var f = {keys:false, onEdit:null, onSuccess:null, afterSave:null, onError:null, afterRestore:null, extraparam:{}, url:null, delOptions:{}, editOptions:{}};
		a = b.jgrid.jqID(a);
		c = b.jgrid.jqID(c);
	...
	b.fn.fmatter.actions = function (a, c) {
		...
		if (d.editformbutton) {
			g = "onclick=jQuery.fn.fmatter.rowactions('" + e + "','" + c.gid + "','formedit'," + c.pos + "); onmouseover=jQuery(this).addClass('ui-state-hover'); onmouseout=jQuery(this).removeClass('ui-state-hover'); ";
			f = f + "<div title='" + b.jgrid.nav.edittitle + "' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' " + g + "><span class='ui-icon ui-icon-pencil'></span></div>";
		} else {
			if (d.editbutton) {
				g = "onclick=jQuery.fn.fmatter.rowactions('" + e + "','" + c.gid + "','edit'," + c.pos + "); onmouseover=jQuery(this).addClass('ui-state-hover'); onmouseout=jQuery(this).removeClass('ui-state-hover') ";
				f = f + "<div title='" + b.jgrid.nav.edittitle + "' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' " + g + "><span class='ui-icon ui-icon-pencil'></span></div>";
			}
		}
		if (d.delbutton) {
	...
	b.unformat = function (a, c, d, e) {
		...
		if (typeof j !== "undefined" && b.isFunction(j)) {
			...
			if (!b.fmatter.isUndefined(g) && b.fmatter.isString(g)) {
				f = b.jgrid.formatter || {};
				switch (g) {
				  case "integer":
		...
		return f !== undefined ? f : e === true ? b(a).text() : b.jgrid.htmlDecode(b(a).html());
	};
	b.unformat.select = function (a, c, d, e) {
		...
		if (c.value) {
			...
			if (b.fmatter.isString(f)) {
				...
				if (b.fmatter.isObject(f) || b.isArray(f)) {
					c || (e[0] = a);
...
(function (a) {
	a.jgrid.extend({getColProp:function (b) {
		var e = {}, c = this[0];
		if (!c.grid) {
			return false;
		}
		...
		return this.each(function () {
			...
			if (this.grid) {
				...
				for (var k = 0; k < this.p.colModel.length; k++) {
					if (this.p.colModel[k].index == b || this.p.colModel[k].name == b) {
						j = k;
						break;
					}
				}
		...
		b = a.extend({autosearch:true, searchOnEnter:true, beforeSearch:null, afterSearch:null, beforeClear:null, afterClear:null, searchurl:"", stringResult:false, groupOp:"AND", defaultSearch:"bw"}, b || {});
		return this.each(function () {
			function e(d, g) {
				var h = a(d);
						...
						switch (this.stype) {
						  case "select":
							if (i = this.surl || f.dataUrl) {
								a.ajax(a.extend({url:i, dataType:"html", success:function (o) {
									if (f.buildSelect !== undefined) {
										(o = f.buildSelect(o)) && a(g).append(o);
									} else {
										a(g).append(o);
									}
								...
								var m;
								if (d.searchoptions && d.searchoptions.value) {
									m = d.searchoptions.value;
								} else {
									if (d.editoptions && d.editoptions.value) {
						  ...
						  case "text":
							...
							if (b.autosearch === true) {
								b.searchOnEnter ? a("input", g).keypress(function (o) {
		...
		return this.each(function () {
			...
			if (q.html() === null) {
				q = a("<tr>", {role:"row", "aria-hidden":"true"}).addClass("jqg-first-row-header").css("height", "auto");
			} else {
				q.empty();
			}
				...
				if (d >= 0) {
					...
					if (k === 0) {
						if (b.useColSpanStyle) {
							...
							a("<th>", {role:"columnheader"}).addClass("ui-state-default ui-th-column-header ui-th-" + e.p.direction).css({display:j.hidden ? "none" : "", "border-top":"0px none"}).insertBefore(h);
							p.append(g);
						}
					} else {
						p.append(g);
			...
			if (b.useColSpanStyle) {
				l.find("span.ui-jqgrid-resize").each(function () {
					var s = a(this).parent();
					if (s.is(":visible")) {
						this.style.cssText = "height: " + s.height() + "px !important; cursor: col-resize;";
					}
				});
...
(function (a) {
	a.extend(a.jgrid, {showModal:function (b) {
		...
		a("a.ui-jqdialog-titlebar-close", i).click(function () {
			...
			q.hideModal("#" + b.themodal, {gb:s, jqm:d.jqModal, onClose:o});
			return false;
		});
		...
		d.closeOnEscape === true && a(e).keydown(function (o) {
			if (o.which == 27) {
				...
				q.hideModal(this, {gb:d.gbox, jqm:d.jqModal, onClose:o});
			}
		});
	}, viewModal:function (b, c) {
		c = a.extend({toTop:true, overlay:10, modal:false, overlayClass:"ui-widget-overlay", onShow:this.showModal, onHide:this.closeModal, gbox:"", jqm:true, jqM:true}, c || {});
		if (a.fn.jqm && c.jqm === true) {
		...
		var g = {width:290, height:"auto", dataheight:"auto", drag:true, resize:false, caption:"<b>" + b + "</b>", left:250, top:170, zIndex:1000, jqModal:true, modal:false, closeOnEscape:true, align:"center", buttonalign:"center", buttons:[]};
		a.extend(g, f || {});
		...
		switch (b) {
		  ...
		  case "select":
			...
			if (typeof c.dataUrl != "undefined") {
				a.ajax(a.extend({url:c.dataUrl, type:"GET", dataType:"html", context:{elem:e, options:c, vl:d}, success:function (m) {
					var l = [], o = this.elem, s = this.vl, p = a.extend({}, this.options), t = p.multiple === true;
					...
					if (m = a(m).html()) {
						...
						setTimeout(function () {
							a("option", o).each(function () {
								...
								if (a.inArray(a.trim(a(this).text()), l) > -1 || a.inArray(a.trim(a(this).val()), l) > -1) {
				...
				if (c.value) {
					...
					if (typeof c.value === "string") {
						...
						for (k = 0; k < q.length; k++) {
							...
							g.innerHTML = i[1];
							e.appendChild(g);
							if (!b && (a.trim(i[0]) == a.trim(d) || a.trim(i[1]) == a.trim(d))) {
								g.selected = "selected";
							}
							if (b && (a.inArray(a.trim(i[1]), f) > -1 || a.inArray(a.trim(i[0]), f) > -1)) {
								g.selected = "selected";
							}
						}
					} else {
						if (typeof c.value === "object") {
							...
							for (k in n) {
								if (n.hasOwnProperty(k)) {
									...
									g.innerHTML = n[k];
									e.appendChild(g);
									if (!b && (a.trim(k) == a.trim(d) || a.trim(n[k]) == a.trim(d))) {
										g.selected = "selected";
									}
									if (b && (a.inArray(a.trim(n[k]), f) > -1 || a.inArray(a.trim(k), f) > -1)) {
										g.selected = "selected";
									}
								}
							}
			...
			try {
				if (a.isFunction(c.custom_element)) {
					if (n = c.custom_element.call(this, d, c)) {
						n = a(n).addClass("customelement").attr({id:c.id, name:c.name});
						a(e).empty().append(n);
					} else {
						throw "e2";
					}
		...
		if (h === -1) {
			...
			k = d[b[h]].toString();
			if (k.length < 1 || d[b[h]] < 1 || d[b[h]] > 31 || d[b[j]] == 2 && d[b[h]] > (d[b[f]] % 4 === 0 && (d[b[f]] % 100 !== 0 || d[b[f]] % 400 === 0) ? 29 : 28) || d[b[h]] > e[d[b[j]]]) {
				return false;
			}
		}
		return true;
		...
		if (h) {
			...
			if (typeof h.minValue != "undefined" && !isNaN(h.minValue)) {
				if (parseFloat(b) < parseFloat(h.minValue)) {
					return [false, j + ": " + a.jgrid.edit.msg.minValue + " " + h.minValue, ""];
				}
			}
			if (typeof h.maxValue != "undefined" && !isNaN(h.maxValue)) {
				if (parseFloat(b) > parseFloat(h.maxValue)) {
					return [false, j + ": " + a.jgrid.edit.msg.maxValue + " " + h.maxValue, ""];
				}
			}
			if (h.email === true) {
				if (!(f === false && this.isEmpty(b))) {
			...
			if (h.date === true) {
				if (!(f === false && this.isEmpty(b))) {
					...
					if (!this.checkDate(c, b)) {
						return [false, j + ": " + a.jgrid.edit.msg.date + " - " + c, ""];
					}
				}
			}
			if (h.time === true) {
				if (!(f === false && this.isEmpty(b))) {
					if (!this.checkTime(b)) {
						return [false, j + ": " + a.jgrid.edit.msg.date + " - hh:mm (am/pm)", ""];
					}
				}
			}
			if (h.url === true) {
	...
	a.jgrid.extend({searchGrid:function (f) {
		...
		return this.each(function () {
			function b() {
				...
				if (x) {
					a.jgrid.viewModal("#" + G.themodal, {gbox:"#gbox_" + q, jqm:f.jqModal, modal:f.modal, overlay:f.overlay, toTop:f.toTop});
					a.isFunction(f.afterShowSearch) && f.afterShowSearch(a("#" + q));
				}
			}
			var e = this;
			if (e.grid) {
				var q = "fbox_" + e.p.id, x = true, G = {themodal:"searchmod" + q, modalhead:"searchhd" + q, modalcontent:"searchcnt" + q, scrollelm:q}, B = e.p.postData[f.sFilter];
				if (typeof B === "string") {
					B = a.jgrid.parse(B);
				}
				...
				if (a("#" + G.themodal).html() !== null) {
					...
					if (f.showQuery) {
						t = "<a href='javascript:void(0)' id='" + q + "_query' class='fm-button ui-state-default ui-corner-all fm-button-icon-left'><span class='ui-icon ui-icon-comment'></span>Query</a>";
					}
					if (f.columns.length) {
						s = f.columns;
					} else {
					...
					if (!B && i || f.multipleSearch === false) {
						var J = "eq";
						if (H >= 0 && s[H].searchoptions && s[H].searchoptions.sopt) {
							J = s[H].searchoptions.sopt[0];
						} else {
							if (f.sopt && f.sopt.length) {
								J = f.sopt[0];
							}
						}
						B = {groupOp:"AND", rules:[{field:i, op:J, data:""}]};
					}
					m = false;
					...
					a("#" + q).jqFilter({columns:s, filter:f.loadDefaults ? B : null, showQuery:f.showQuery, errorcheck:f.errorcheck, sopt:f.sopt, groupButton:f.multipleGroup, ruleButtons:f.multipleSearch, afterRedraw:f.afterRedraw, _gridsopt:a.jgrid.search.odata, ajaxSelectOptions:e.p.ajaxSelectOptions, onChange:function () {
						this.p.showQuery && a(".query", this).html(this.toUserFriendlyString());
					}, direction:e.p.direction});
					w.append(n);
					m && f.tmplFilters && f.tmplFilters.length && a(".ui-template", w).bind("change", function () {
						var u = a(this).val();
					...
					f.gbox = "#gbox_" + q;
					f.layer ? a.jgrid.createModal(G, w, f, "#gview_" + e.p.id, a("#gbox_" + e.p.id)[0], "#" + f.layer, {position:"relative"}) : a.jgrid.createModal(G, w, f, "#gview_" + e.p.id, a("#gbox_" + e.p.id)[0]);
					...
					a("#" + q + "_search").bind("click", function () {
						...
						f.closeAfterSearch && a.jgrid.hideModal("#" + G.themodal, {gb:"#gbox_" + e.p.id, jqm:f.jqModal, onClose:f.onClose});
						return false;
					});
		...
		return this.each(function () {
			function e() {
				a("#" + i + " > tbody > tr > td > .FormElement").each(function () {
					...
					if (g.length) {
						...
						if (c.p.autoencode) {
							h[this.name] = a.jgrid.htmlEncode(h[this.name]);
						}
					}
				});
				return true;
				...
				a(k.p.colModel).each(function (S) {
					...
					if (E !== "cb" && E !== "subgrid" && this.editable === true && E !== "rn") {
						if (I === false) {
							...
							if (E == k.p.ExpandColumn && k.p.treeGrid === true) {
								r = a("td:eq(" + S + ")", k.rows[I]).text();
							} else {
								try {
									r = a.unformat(a("td:eq(" + S + ")", k.rows[I]), {rowId:g, colModel:this}, S);
								}
								catch (ea) {
									r = this.edittype && this.edittype == "textarea" ? a("td:eq(" + S + ")", k.rows[I]).text() : a("td:eq(" + S + ")", k.rows[I]).html();
								}
								if (!r || r == "&nbsp;" || r == "&#160;" || r.length == 1 && r.charCodeAt(0) == 160) {
									r = "";
								}
							}
						}
						var X = a.extend({}, this.editoptions || {}, {id:E, name:E}), Y = a.extend({}, {elmprefix:"", elmsuffix:"", rowabove:false, rowcontent:""}, this.formoptions || {}), da = parseInt(Y.rowpos, 10) || L + 1, fa = parseInt((parseInt(Y.colpos, 10) || 1) * 2, 10);
						if (g == "_empty" && X.defaultValue) {
							r = a.isFunction(X.defaultValue) ? X.defaultValue() : X.defaultValue;
						}
						if (!this.edittype) {
							this.edittype = "text";
						}
						...
						if (this.edittype == "text" || this.edittype == "textarea") {
							a(F).addClass("ui-widget-content ui-corner-all");
						}
						...
						if ((z = a("#" + a.jgrid.jqID(p), "#" + v)) && z.length && z[0] !== null) {
							...
							if (L.defaultValue) {
								...
								if (z[0].type == "checkbox") {
									...
									if (z[0].type && z[0].type.substr(0, 6) == "select") {
										z[0].selectedIndex = 0;
									} else {
										z.val(r);
									}
							...
							if (d[c.p.id].checkOnSubmit === true || d[c.p.id].checkOnUpdate) {
					...
					if (T) {
						a("td[role=\"gridcell\"]", T).each(function (I) {
							...
							if (p !== "cb" && p !== "subgrid" && p !== "rn" && F[I].editable === true) {
								if (p == k.p.ExpandColumn && k.p.treeGrid === true) {
									D = a(this).text();
								} else {
									try {
										D = a.unformat(a(this), {rowId:g, colModel:F[I]}, I);
									}
									catch (ca) {
								...
								switch (F[I].edittype) {
								  ...
								  case "checkbox":
									D += "";
									if (F[I].editoptions && F[I].editoptions.value) {
										if (F[I].editoptions.value.split(":")[0] == D) {
								  ...
								  case "custom":
									try {
										if (F[I].editoptions && a.isFunction(F[I].editoptions.custom_value)) {
											F[I].editoptions.custom_value(a("#" + p, "#" + v), "set", D);
										} else {
											throw "e1";
										}
									}
						...
						for (r in c.p.treeReader) {
							if (c.p.treeReader.hasOwnProperty(r)) {
								...
								if (h.hasOwnProperty(v)) {
									h[D] == p.addoper && r === "parent_id_field" || delete h[v];
								}
							}
						}
					}
					h[E] = a.jgrid.stripPref(c.p.idPrefix, h[E]);
					r = a.extend({url:d[c.p.id].url ? d[c.p.id].url : a(c).jqGrid("getGridParam", "editurl"), type:d[c.p.id].mtype, data:a.isFunction(d[c.p.id].serializeEditData) ? d[c.p.id].serializeEditData(h) : h, complete:function (F, T) {
						...
						if (T != "success") {
							...
							if (a.isFunction(d[c.p.id].afterSubmit)) {
						...
						if (k[0] === false) {
							...
							a.each(c.p.colModel, function () {
								if (O[this.name] && this.formatter && this.formatter == "select") {
									try {
										delete O[this.name];
									}
									catch (P) {
							...
							if (h[D] == p.addoper) {
								...
								if (d[c.p.id].closeAfterAdd) {
									if (d[c.p.id].reloadAfterSubmit) {
										...
										if (c.p.treeGrid === true) {
											a(c).jqGrid("addChildNode", k[2], L, h);
										} else {
											a(c).jqGrid("addRowData", k[2], h, b.addedrow);
											a(c).jqGrid("setSelection", k[2]);
										}
									}
									a.jgrid.hideModal("#" + m.themodal, {gb:"#gbox_" + t, jqm:b.jqModal, onClose:d[c.p.id].onClose});
								} else {
									if (d[c.p.id].clearAfterAdd) {
										if (d[c.p.id].reloadAfterSubmit) {
											a(c).trigger("reloadGrid");
										} else {
											c.p.treeGrid === true ? a(c).jqGrid("addChildNode", k[2], L, h) : a(c).jqGrid("addRowData", k[2], h, b.addedrow);
										}
										x("_empty", c, o);
									} else {
										if (d[c.p.id].reloadAfterSubmit) {
								...
								if (d[c.p.id].reloadAfterSubmit) {
									...
									c.p.treeGrid === true ? a(c).jqGrid("setTreeRow", h[E], h) : a(c).jqGrid("setRowData", h[E], h);
								}
								d[c.p.id].closeAfterEdit && a.jgrid.hideModal("#" + m.themodal, {gb:"#gbox_" + t, jqm:b.jqModal, onClose:d[c.p.id].onClose});
							}
					...
					if (k[0]) {
						if (d[c.p.id].useDataProxy) {
							...
							if (v[0] === false) {
								...
								r.data.oper == p.addoper && d[c.p.id].closeAfterAdd && a.jgrid.hideModal("#" + m.themodal, {gb:"#gbox_" + t, jqm:b.jqModal, onClose:d[c.p.id].onClose});
								r.data.oper == p.editoper && d[c.p.id].closeAfterEdit && a.jgrid.hideModal("#" + m.themodal, {gb:"#gbox_" + t, jqm:b.jqModal, onClose:d[c.p.id].onClose});
							}
						} else {
							a.ajax(r);
						}
			...
			function j() {
				if (f !== "_empty" && typeof c.p.savedRow !== "undefined" && c.p.savedRow.length > 0 && a.isFunction(a.fn.jqGrid.restoreRow)) {
			...
			var c = this;
			if (c.grid && f) {
				var t = c.p.id, o = "FrmGrid_" + t, i = "TblGrid_" + t, m = {themodal:"editmod" + t, modalhead:"edithd" + t, modalcontent:"editcnt" + t, scrollelm:o}, H = a.isFunction(d[c.p.id].beforeShowForm) ? d[c.p.id].beforeShowForm : false, J = a.isFunction(d[c.p.id].afterShowForm) ? d[c.p.id].afterShowForm : false, u = a.isFunction(d[c.p.id].beforeInitData) ? d[c.p.id].beforeInitData : false, C = a.isFunction(d[c.p.id].onInitializeForm) ? d[c.p.id].onInitializeForm : false, A = true, l = 1, y = 0, h, O, N, U;
				if (f === "new") {
					f = "_empty";
				...
				if (a("#" + m.themodal).html() !== null) {
					...
					a.jgrid.viewModal("#" + m.themodal, {gbox:"#gbox_" + t, jqm:b.jqModal, jqM:false, overlay:b.overlay, modal:b.modal});
					...
					var $ = "<a href='javascript:void(0)' id='sData' class='fm-button ui-state-default ui-corner-all'>" + b.bSubmit + "</a>", Z = "<a href='javascript:void(0)' id='cData' class='fm-button ui-state-default ui-corner-all'>" + b.bCancel + "</a>";
					...
					a("#" + m.themodal).keydown(function (g) {
						...
						if (d[c.p.id].savekey[0] === true && g.which == d[c.p.id].savekey[1]) {
						...
						if (g.which === 27) {
							...
							ba && a.jgrid.hideModal(this, {gb:b.gbox, jqm:b.jqModal, onClose:d[c.p.id].onClose});
							return false;
						}
					...
					if (b.saveicon[0] === true) {
						a("#sData", "#" + i + "_2").addClass(b.saveicon[1] == "right" ? "fm-button-icon-right" : "fm-button-icon-left").append("<span class='ui-icon " + b.saveicon[2] + "'></span>");
					}
					if (b.closeicon[0] === true) {
						a("#cData", "#" + i + "_2").addClass(b.closeicon[1] == "right" ? "fm-button-icon-right" : "fm-button-icon-left").append("<span class='ui-icon " + b.closeicon[2] + "'></span>");
					}
					if (d[c.p.id].checkOnSubmit || d[c.p.id].checkOnUpdate) {
					...
					H && H(a("#" + o));
					a("#" + m.themodal).data("onClose", d[c.p.id].onClose);
					a.jgrid.viewModal("#" + m.themodal, {gbox:"#gbox_" + t, jqm:b.jqModal, overlay:b.overlay, modal:b.modal});
					Q || a(".jqmOverlay").click(function () {
						if (!n()) {
							return false;
						}
					...
					a("#nData", "#" + i + "_2").click(function () {
						...
						if (g[0] != -1 && g[1][g[0] + 1]) {
							if (a.isFunction(b.onclickPgButtons)) {
								b.onclickPgButtons("next", a("#" + o), g[1][g[0]]);
							}
							x(g[1][g[0] + 1], c, o);
					...
					a("#pData", "#" + i + "_2").click(function () {
						...
						if (g[0] != -1 && g[1][g[0] - 1]) {
							if (a.isFunction(b.onclickPgButtons)) {
								b.onclickPgButtons("prev", a("#" + o), g[1][g[0]]);
							}
							x(g[1][g[0] - 1], c, o);
							a(c).jqGrid("setSelection", g[1][g[0] - 1]);
		...
		b = a.extend({top:0, left:0, width:0, height:"auto", dataheight:"auto", modal:false, overlay:30, drag:true, resize:true, jqModal:true, closeOnEscape:false, labelswidth:"30%", closeicon:[], navkeys:[false, 38, 40], onClose:null, beforeShowForm:null, beforeInitData:null, viewPagerButtons:true}, a.jgrid.view, b || {});
		return this.each(function () {
			function e() {
			...
			function q(l, y, h, O) {
				for (var N, U, Q, K = 0, V, W, $ = [], Z = false, aa = "<td class='CaptionTD form-view-label ui-widget-content' width='" + b.labelswidth + "'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content'>&#160;</td>", ba = "", g = ["integer", "number", "currency"], k = 0, v = 0, p, E, D, z = 1; z <= O; z++) {
					ba += z == 1 ? aa : "<td class='CaptionTD form-view-label ui-widget-content'>&#160;</td><td class='DataTD form-view-data ui-widget-content'>&#160;</td>";
				}
				a(y.p.colModel).each(function () {
					...
					if (!U && this.align === "right") {
						if (this.formatter && a.inArray(this.formatter, g) !== -1) {
				...
				a(y.p.colModel).each(function (L) {
					...
					if (N !== "cb" && N !== "subgrid" && N !== "rn" && D) {
						...
						a.extend({}, this.editoptions || {}, {id:N, name:N});
						var r = a.extend({}, {rowabove:false, rowcontent:""}, this.formoptions || {}), R = parseInt(r.rowpos, 10) || K + 1, F = parseInt((parseInt(r.colpos, 10) || 1) * 2, 10);
						if (r.rowabove) {
						...
						E && a("td:eq(" + (F - 1) + ") span", Q[0]).css({"text-align":"right", width:p + "px"});
						$[K] = L;
						K++;
					}
				});
			...
			var w = this;
			if (w.grid && f) {
				var n = w.p.id, j = "ViewGrid_" + n, s = "ViewTbl_" + n, M = {themodal:"viewmod" + n, modalhead:"viewhd" + n, modalcontent:"viewcnt" + n, scrollelm:j}, c = a.isFunction(b.beforeInitData) ? b.beforeInitData : false, t = true, o = 1, i = 0;
				if (a("#" + M.themodal).html() !== null) {
					if (c) {
						t = c(a("#" + j));
					...
					a.jgrid.viewModal("#" + M.themodal, {gbox:"#gbox_" + n, jqm:b.jqModal, jqM:false, overlay:b.overlay, modal:b.modal});
					e();
				} else {
					...
					a("#" + M.themodal).keydown(function (l) {
						if (l.which === 27) {
							A && a.jgrid.hideModal(this, {gb:b.gbox, jqm:b.jqModal, onClose:b.onClose});
							return false;
						}
						if (b.navkeys[0] === true) {
					...
					b.closeicon = a.extend([true, "left", "ui-icon-close"], b.closeicon);
					if (b.closeicon[0] === true) {
						a("#cData", "#" + s + "_2").addClass(b.closeicon[1] == "right" ? "fm-button-icon-right" : "fm-button-icon-left").append("<span class='ui-icon " + b.closeicon[2] + "'></span>");
					}
					a.isFunction(b.beforeShowForm) && b.beforeShowForm(a("#" + j));
					a.jgrid.viewModal("#" + M.themodal, {gbox:"#gbox_" + n, jqm:b.jqModal, modal:b.modal});
					...
					a("#cData", "#" + s + "_2").click(function () {
						a.jgrid.hideModal("#" + M.themodal, {gb:"#gbox_" + n, jqm:b.jqModal, onClose:b.onClose});
						return false;
					});
					a("#nData", "#" + s + "_2").click(function () {
						...
						if (l[0] != -1 && l[1][l[0] + 1]) {
							if (a.isFunction(b.onclickPgButtons)) {
								b.onclickPgButtons("next", a("#" + j), l[1][l[0]]);
							}
							x(l[1][l[0] + 1], w);
					...
					a("#pData", "#" + s + "_2").click(function () {
						...
						if (l[0] != -1 && l[1][l[0] - 1]) {
							if (a.isFunction(b.onclickPgButtons)) {
								b.onclickPgButtons("prev", a("#" + j), l[1][l[0]]);
							}
							x(l[1][l[0] - 1], w);
							a(w).jqGrid("setSelection", l[1][l[0] - 1]);
		...
		b = a.extend({top:0, left:0, width:240, height:"auto", dataheight:"auto", modal:false, overlay:30, drag:true, resize:true, url:"", mtype:"POST", reloadAfterSubmit:true, beforeShowForm:null, beforeInitData:null, afterShowForm:null, beforeSubmit:null, onclickSubmit:null, afterSubmit:null, jqModal:true, closeOnEscape:false, delData:{}, delicon:[], cancelicon:[], onClose:null, ajaxDelOptions:{}, processing:false, serializeDelData:null, useDataProxy:false}, a.jgrid.del, b || {});
		d[a(this)[0].p.id] = b;
		return this.each(function () {
			var e = this;
			if (e.grid) {
				if (f) {
					var q = a.isFunction(d[e.p.id].beforeShowForm), x = a.isFunction(d[e.p.id].afterShowForm), G = a.isFunction(d[e.p.id].beforeInitData) ? d[e.p.id].beforeInitData : false, B = e.p.id, w = {}, n = true, j = "DelTbl_" + B, s, M, c, t, o = {themodal:"delmod" + B, modalhead:"delhd" + B, modalcontent:"delcnt" + B, scrollelm:j};
					if (jQuery.isArray(f)) {
						f = f.join();
					}
					if (a("#" + o.themodal).html() !== null) {
						...
						a.jgrid.viewModal("#" + o.themodal, {gbox:"#gbox_" + B, jqm:d[e.p.id].jqModal, jqM:false, overlay:d[e.p.id].overlay, modal:d[e.p.id].modal});
					} else {
						...
						if (b.delicon[0] === true) {
							a("#dData", "#" + j + "_2").addClass(b.delicon[1] == "right" ? "fm-button-icon-right" : "fm-button-icon-left").append("<span class='ui-icon " + b.delicon[2] + "'></span>");
						}
						if (b.cancelicon[0] === true) {
							a("#eData", "#" + j + "_2").addClass(b.cancelicon[1] == "right" ? "fm-button-icon-right" : "fm-button-icon-left").append("<span class='ui-icon " + b.cancelicon[2] + "'></span>");
						}
							...
							if (m[0] && !d[e.p.id].processing) {
								...
								for (var J in H) {
									if (H.hasOwnProperty(J)) {
										H[J] = a.jgrid.stripPref(e.p.idPrefix, H[J]);
									}
								}
								s[M] = H.join();
								J = a.extend({url:d[e.p.id].url ? d[e.p.id].url : a(e).jqGrid("getGridParam", "editurl"), type:d[e.p.id].mtype, data:a.isFunction(d[e.p.id].serializeDelData) ? d[e.p.id].serializeDelData(s) : s, complete:function (u, C) {
									if (C != "success") {
										m[0] = false;
										m[1] = a.isFunction(d[e.p.id].errorTextFormat) ? d[e.p.id].errorTextFormat(u) : C + " Status: '" + u.statusText + "'. Error code: " + u.status;
									} else {
										if (a.isFunction(d[e.p.id].afterSubmit)) {
											m = d[e.p.id].afterSubmit(u, s);
										}
									}
									if (m[0] === false) {
										...
										if (d[e.p.id].reloadAfterSubmit && e.p.datatype != "local") {
											...
											if (e.p.treeGrid === true) {
												...
												for (var y = 0; y < A.length; y++) {
													a(e).jqGrid("delRowData", e.p.idPrefix + A[y]);
												}
											}
											e.p.selrow = null;
											e.p.selarrrow = [];
									...
									m[0] && a.jgrid.hideModal("#" + o.themodal, {gb:"#gbox_" + B, jqm:b.jqModal, onClose:d[e.p.id].onClose});
								...
								if (m[0]) {
									if (d[e.p.id].useDataProxy) {
										...
										if (J[0] === false) {
											...
											a.jgrid.hideModal("#" + o.themodal, {gb:"#gbox_" + B, jqm:b.jqModal, onClose:d[e.p.id].onClose});
										}
									} else {
										a.ajax(J);
									}
						...
						a("#eData", "#" + j + "_2").click(function () {
							a.jgrid.hideModal("#" + o.themodal, {gb:"#gbox_" + B, jqm:d[e.p.id].jqModal, onClose:d[e.p.id].onClose});
							return false;
						});
						q && d[e.p.id].beforeShowForm(a("#" + j));
						a.jgrid.viewModal("#" + o.themodal, {gbox:"#gbox_" + B, jqm:d[e.p.id].jqModal, overlay:d[e.p.id].overlay, modal:d[e.p.id].modal});
					}
		...
		return this.each(function () {
			if (!this.nav) {
				var w = {themodal:"alertmod", modalhead:"alerthd", modalcontent:"alertcnt"}, n = this, j;
				if (!(!n.grid || typeof f != "string")) {
					if (a("#" + w.themodal).html() === null) {
						...
						a.jgrid.createModal(w, "<div>" + b.alerttext + "</div><span tabindex='0'><span tabindex='-1' id='jqg_alrt'></span></span>", {gbox:"#gbox_" + n.p.id, jqModal:true, drag:true, resize:true, caption:b.alertcap, top:b.alerttop, left:b.alertleft, width:b.alertwidth, height:b.alertheight, closeOnEscape:b.closeOnEscape, zIndex:b.alertzIndex}, "", "", true);
					}
					var s = 1;
					...
					for (var M = 0; M < s; M++) {
						...
						if (b.add) {
							...
							a("tr", c).append(j);
							a(j, c).attr({title:b.addtitle || "", id:q.id || "add_" + o}).click(function () {
						...
						if (b.edit) {
							...
							a("tr", c).append(j);
							a(j, c).attr({title:b.edittitle || "", id:e.id || "edit_" + o}).click(function () {
								if (!a(this).hasClass("ui-state-disabled")) {
									var i = n.p.selrow;
									if (i) {
										a.isFunction(b.editfunc) ? b.editfunc(i) : a(n).jqGrid("editGridRow", i, e);
									} else {
						...
						if (b.view) {
							...
							a("tr", c).append(j);
							a(j, c).attr({title:b.viewtitle || "", id:B.id || "view_" + o}).click(function () {
								if (!a(this).hasClass("ui-state-disabled")) {
									var i = n.p.selrow;
									if (i) {
										a.isFunction(b.viewfunc) ? b.viewfunc(i) : a(n).jqGrid("viewGridRow", i, B);
									} else {
										a.jgrid.viewModal("#" + w.themodal, {gbox:"#gbox_" + n.p.id, jqm:true});
										a("#jqg_alrt").focus();
									}
						...
						if (b.del) {
							...
							a("tr", c).append(j);
							a(j, c).attr({title:b.deltitle || "", id:x.id || "del_" + o}).click(function () {
								if (!a(this).hasClass("ui-state-disabled")) {
									var i;
									if (n.p.multiselect) {
										i = n.p.selarrrow;
									...
									if (i) {
										"function" == typeof b.delfunc ? b.delfunc(i) : a(n).jqGrid("delGridRow", i, x);
									} else {
										a.jgrid.viewModal("#" + w.themodal, {gbox:"#gbox_" + n.p.id, jqm:true});
										a("#jqg_alrt").focus();
									}
						...
						if (b.add || b.edit || b.del || b.view) {
							a("tr", c).append("<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='ui-separator'></span></td>");
						}
						if (b.search) {
							j = a("<td class='ui-pg-button ui-corner-all'></td>");
							G = G || {};
							...
							a("tr", c).append(j);
							a(j, c).attr({title:b.searchtitle || "", id:G.id || "search_" + o}).click(function () {
						...
						if (b.refresh) {
							...
							a("tr", c).append(j);
							a(j, c).attr({title:b.refreshtitle || "", id:"refresh_" + o}).click(function () {
								if (!a(this).hasClass("ui-state-disabled")) {
									...
									switch (b.refreshstate) {
									  case "firstpage":
										...
										break;
									  case "current":
										a(n).trigger("reloadGrid", [{current:true}]);
									}
			...
			if (this.grid) {
				...
				if (e) {
					if (!(b.id && a("#" + b.id, e).html() !== null)) {
						...
						if (b.position == "first") {
							e.rows[0].cells.length === 0 ? a("tr", e).append(x) : a("tr td:eq(0)", e).before(x);
						} else {
							a("tr", e).append(x);
						}
		...
		return this.each(function () {
			...
			if (e.grid) {
				...
				if (q) {
					for (var x in q) {
						a("[name=" + a.jgrid.jqID(x) + "]", b).is("input:radio") || a("[name=" + a.jgrid.jqID(x) + "]", b).is("input:checkbox") ? a("[name=" + a.jgrid.jqID(x) + "]", b).each(function () {
							if (a(this).val() == q[x]) {
								a(this)[e.p.useProp ? "prop" : "attr"]("checked", true);
							} else {
								a(this)[e.p.useProp ? "prop" : "attr"]("checked", false);
							}
		...
		return this.each(function () {
			if (this.grid) {
				...
				if (e == "add") {
					...
					e == "set" && a(this).jqGrid("setRowData", f, G);
				}
			}
		});
	}});
...
(function (c) {
	c.fn.jqFilter = function (k) {
		...
		var o = c.extend(true, {filter:null, columns:[], onChange:null, afterRedraw:null, checkValues:null, error:false, errmsg:"", errorcheck:true, showQuery:true, sopt:null, ops:[{name:"eq", description:"equal", operator:"="}, {name:"ne", description:"not equal", operator:"<>"}, {name:"lt", description:"less", operator:"<"}, {name:"le", description:"less or equal", operator:"<="}, {name:"gt", description:"greater", operator:">"}, {name:"ge", description:"greater or equal", operator:">="}, {name:"bw", description:"begins with", operator:"LIKE"}, {name:"bn", description:"does not begin with", operator:"NOT LIKE"}, {name:"in", description:"in", operator:"IN"}, {name:"ni", description:"not in", operator:"NOT IN"}, {name:"ew", description:"ends with", operator:"LIKE"}, {name:"en", description:"does not end with", operator:"NOT LIKE"}, {name:"cn", description:"contains", operator:"LIKE"}, {name:"nc", description:"does not contain", operator:"NOT LIKE"}, {name:"nu", description:"is null", operator:"IS NULL"}, {name:"nn", description:"is not null", operator:"IS NOT NULL"}], numopts:["eq", "ne", "lt", "le", "gt", "ge", "nu", "nn", "in", "ni"], stropts:["eq", "ne", "bw", "bn", "ew", "en", "cn", "nc", "nu", "nn", "in", "ni"], _gridsopt:[], groupOps:[{op:"AND", text:"AND"}, {op:"OR", text:"OR"}], groupButton:true, ruleButtons:true, direction:"ltr"}, k || {});
		return this.each(function () {
			if (!this.filter) {
				this.p = o;
				...
				if (x) {
					...
					var z = function (d, g) {
						...
						if (c.isFunction(g.searchrules)) {
							...
							if (c.jgrid && c.jgrid.checkValues) {
								try {
					...
					this.createTableForGroup = function (d, g) {
						...
						if (this.p.groupButton) {
							...
							f.bind("click", function () {
								...
								d.groups.push({groupOp:o.groupOps[0].op, rules:[], groups:[]});
								a.reDraw();
								a.onchange();
								return false;
							});
						...
						if (this.p.ruleButtons === true) {
							...
							f.bind("click", function () {
								...
								d.rules.push({field:m.name, op:(m.searchoptions.sopt ? m.searchoptions.sopt : a.p.sopt ? a.p.sopt : m.searchtype === "string" ? a.p.stropts : a.p.numopts)[0], data:""});
								a.reDraw();
								return false;
							});
							i.append(f);
						...
						if (d.rules !== undefined) {
							for (b = 0; b < d.rules.length; b++) {
								e.append(this.createTableRowForRule(d.rules[b], d));
							}
						}
						return e;
					};
	...
	c.extend(c.fn.jqFilter, {toSQLString:function () {
		var k = "";
		this.each(function () {
...
(function (a) {
	...
	a.jgrid.extend({editRow:function (c, d, b, i, q, l, s, m, t) {
		var h = {keys:d || false, oneditfunc:b || null, successfunc:i || null, url:q || null, extraparam:l || {}, aftersavefunc:s || null, errorfunc:m || null, afterrestorefunc:t || null, restoreAfterError:true, mtype:"POST"}, u = a.makeArray(arguments).slice(1), e;
		e = u[0] && typeof u[0] == "object" && !a.isFunction(u[0]) ? a.extend(a.jgrid.inlineEdit, h, u[0]) : h;
		return this.each(function () {
			var g = this, f, o, x = 0, y = null, w = {}, p, j;
			if (g.grid) {
				p = a(g).jqGrid("getInd", c, true);
				if (p !== false) {
					if ((a(p).attr("editable") || "0") == "0" && !a(p).hasClass("not-editable-row")) {
						j = g.p.colModel;
						a("td[role=\"gridcell\"]", p).each(function (k) {
							f = j[k].name;
							var A = g.p.treeGrid === true && f == g.p.ExpandColumn;
							if (A) {
								o = a("span:first", this).html();
							} else {
								try {
									o = a.unformat(this, {rowId:c, colModel:j[k]}, k);
								}
								catch (r) {
									o = j[k].edittype && j[k].edittype == "textarea" ? a(this).text() : a(this).html();
								}
							}
							if (f != "cb" && f != "subgrid" && f != "rn") {
								...
								if (j[k].editable === true) {
									...
									var n = a.extend({}, j[k].editoptions || {}, {id:c + "_" + f, name:f});
									if (!j[k].edittype) {
										j[k].edittype = "text";
									}
									...
									n = a.jgrid.createEl(j[k].edittype, n, o, true, a.extend({}, a.jgrid.ajaxOptions, g.p.ajaxSelectOptions || {}));
									a(n).addClass("editable");
									A ? a("span:first", this).append(n) : a(this).append(n);
									j[k].edittype == "select" && typeof j[k].editoptions !== "undefined" && j[k].editoptions.multiple === true && typeof j[k].editoptions.dataUrl === "undefined" && a.browser.msie && a(n).width(a(n).width());
									x++;
								}
							}
						});
						if (x > 0) {
							...
							e.keys === true && a(p).bind("keydown", function (k) {
								if (k.keyCode === 27) {
							...
							a.isFunction(e.oneditfunc) && e.oneditfunc.call(g, c);
						}
					}
				}
			}
		...
		var m = {successfunc:d || null, url:b || null, extraparam:i || {}, aftersavefunc:q || null, errorfunc:l || null, afterrestorefunc:s || null, restoreAfterError:true, mtype:"POST"}, t = a.makeArray(arguments).slice(1), h;
		h = t[0] && typeof t[0] == "object" && !a.isFunction(t[0]) ? a.extend(a.jgrid.inlineEdit, m, t[0]) : m;
		...
		if (m === "1") {
			...
			a("td[role=\"gridcell\"]", p).each(function (r) {
				j = e.p.colModel[r];
				g = j.name;
				if (g != "cb" && g != "subgrid" && j.editable === true && g != "rn" && !a(this).hasClass("not-editable-cell")) {
					switch (j.edittype) {
					  case "checkbox":
						var n = ["Yes", "No"];
						if (j.editoptions) {
						...
						f[g] = a("input", this).is(":checked") ? n[0] : n[1];
						break;
					  case "text":
					  case "password":
					  case "textarea":
					  ...
					  case "select":
						if (j.editoptions.multiple) {
							...
							f[g] = a("select option:selected", this).val();
							o[g] = a("select option:selected", this).text();
						}
					  ...
					  case "custom":
						try {
							if (j.editoptions && a.isFunction(j.editoptions.custom_value)) {
								f[g] = j.editoptions.custom_value.call(e, a(".customelement", this), "get");
								if (f[g] === undefined) {
									throw "e2";
								}
							} else {
					...
					if (h.url !== "clientArray" && j.editoptions && j.editoptions.NullIfEmpty === true) {
						if (f[g] === "") {
							x[g] = "null";
						}
					}
			...
			if (w[0] === false) {
				try {
					var k = a.jgrid.findPos(a("#" + a.jgrid.jqID(c), e.grid.bDiv)[0]);
					a.jgrid.info_dialog(a.jgrid.errors.errcap, w[1], a.jgrid.edit.bClose, {left:k[0], top:k[1]});
				}
				catch (A) {
					alert(w[1]);
				}
			...
			if (h.url == "clientArray") {
				...
				a.isFunction(h.aftersavefunc) && h.aftersavefunc.call(e, c, k);
				u = true;
				a(p).unbind("keydown");
			} else {
				a("#lui_" + e.p.id).show();
				...
				a.ajax(a.extend({url:h.url, data:a.isFunction(e.p.serializeRowData) ? e.p.serializeRowData.call(e, x) : x, type:h.mtype, async:false, complete:function (r, n) {
					a("#lui_" + e.p.id).hide();
					if (n === "success") {
						var v = true, z;
						...
						if (v === true) {
							e.p.autoencode && a.each(f, function (B, C) {
								f[B] = a.jgrid.htmlDecode(C);
							});
							f = a.extend({}, f, o);
							...
							y >= 0 && e.p.savedRow.splice(y, 1);
							a.isFunction(h.aftersavefunc) && h.aftersavefunc.call(e, c, r);
							u = true;
							a(p).unbind("keydown");
						} else {
							a.isFunction(h.errorfunc) && h.errorfunc.call(e, c, r, n);
							h.restoreAfterError === true && a(e).jqGrid("restoreRow", c, h.afterrestorefunc);
						}
					}
				}, error:function (r, n) {
					a("#lui_" + e.p.id).hide();
					if (a.isFunction(h.errorfunc)) {
						...
						try {
							jQuery.jgrid.info_dialog(jQuery.jgrid.errors.errcap, "<div class=\"ui-state-error\">" + r.responseText + "</div>", jQuery.jgrid.edit.bClose, {buttonalign:"right"});
						}
						catch (v) {
		...
		var b = {afterrestorefunc:d || null}, i = a.makeArray(arguments).slice(1), q;
		q = i[0] && typeof i[0] == "object" && !a.isFunction(i[0]) ? a.extend(a.jgrid.inlineEdit, b, i[0]) : b;
		return this.each(function () {
			var l = this, s, m, t = {};
			if (l.grid) {
				...
				if (m !== false) {
					...
					if (s >= 0) {
						if (a.isFunction(a.fn.datepicker)) {
							try {
								a("input.hasDatepicker", "#" + a.jgrid.jqID(m.id)).datepicker("hide");
							}
							catch (u) {
							}
						}
						a.each(l.p.colModel, function () {
							if (this.editable === true && this.name in l.p.savedRow[s] && !a(this).hasClass("not-editable-cell")) {
					...
					a.isFunction(q.afterrestorefunc) && q.afterrestorefunc.call(l, c);
				}
			}
		});
	}, addRow:function (c) {
		c = a.extend({rowID:"new_row", initdata:{}, position:"first", useDefValues:false, useFormatter:false, addRowParams:{extraparam:{}}}, c || {});
		return this.each(function () {
			if (this.grid) {
				var d = this;
				...
				if (c.useFormatter) {
					a("#" + a.jgrid.jqID(c.rowID) + " .ui-inline-edit", "#" + a.jgrid.jqID(d.p.id)).click();
				} else {
					var b = d.p.prmNames;
		...
		d = a.extend({edit:true, editicon:"ui-icon-pencil", add:true, addicon:"ui-icon-plus", save:true, saveicon:"ui-icon-disk", cancel:true, cancelicon:"ui-icon-cancel", addParams:{useFormatter:false}, editParams:{}}, a.jgrid.nav, d || {});
		return this.each(function () {
			if (this.grid) {
				var b = this;
					...
					for (q = 0; q < i.length; q++) {
						if (i[q].formatter && i[q].formatter === "actions") {
							if (i[q].formatoptions) {
								i = a.extend({keys:false, onEdit:null, onSuccess:null, afterSave:null, onError:null, afterRestore:null, extraparam:{}, url:null}, i[q].formatoptions);
								d.addParams.addRowParams = {keys:i.keys, oneditfunc:i.onEdit, successfunc:i.onSuccess, url:i.url, extraparam:i.extraparam, aftersavefunc:i.afterSavef, errorfunc:i.onError, afterrestorefunc:i.afterRestore};
							}
							break;
						}
					}
				}
				d.add && a(b).jqGrid("navButtonAdd", c, {caption:d.addtext, title:d.addtitle, buttonicon:d.addicon, id:b.p.id + "_iladd", onClickButton:function () {
				...
				d.edit && a(b).jqGrid("navButtonAdd", c, {caption:d.edittext, title:d.edittitle, buttonicon:d.editicon, id:b.p.id + "_iledit", onClickButton:function () {
					...
					if (l) {
						...
						a.jgrid.viewModal("#alertmod", {gbox:"#gbox_" + b.p.id, jqm:true});
						a("#jqg_alrt").focus();
					}
				}});
				if (d.save) {
					a(b).jqGrid("navButtonAdd", c, {caption:d.savetext || "", title:d.savetitle || "Save row", buttonicon:d.saveicon, id:b.p.id + "_ilsave", onClickButton:function () {
						var l = a(b).jqGrid("getGridParam", "selrow");
						if (l) {
							if (a("#" + a.jgrid.jqID(l), "#" + a.jgrid.jqID(b.p.id)).hasClass("jqgrid-new-row")) {
				...
				if (d.cancel) {
					a(b).jqGrid("navButtonAdd", c, {caption:d.canceltext || "", title:d.canceltitle || "Cancel row editing", buttonicon:d.cancelicon, id:b.p.id + "_ilcancel", onClickButton:function () {
						var l = a(b).jqGrid("getGridParam", "selrow");
						if (l) {
			...
			if (!(!c.grid || c.p.cellEdit !== true)) {
				...
				if (!(h == "subgrid" || h == "cb" || h == "rn")) {
					...
					if (i.editable === true && a === true && !g.hasClass("not-editable-cell")) {
						...
						if (e == "&nbsp;" || e == "&#160;" || e.length == 1 && e.charCodeAt(0) == 160) {
							e = "";
						}
						...
						b("input, select, textarea", g).bind("keydown", function (l) {
							...
							if (l.keyCode == 9) {
								if (c.grid.hDiv.loading) {
									...
									l.shiftKey ? b(c).jqGrid("prevCell", d, f) : b(c).jqGrid("nextCell", d, f);
								}
							}
							l.stopPropagation();
						});
				...
				if (c !== null) {
					...
					if (g !== a.p.savedRow[c].v) {
						if (b.isFunction(a.p.beforeSaveCell)) {
							if (c = a.p.beforeSaveCell.call(a, a.rows[d].id, k, e, d, f)) {
								g = e = c;
							}
						}
						...
						if (r[0] === true) {
							...
							if (b.isFunction(a.p.beforeSubmitCell)) {
								(c = a.p.beforeSubmitCell.call(a, a.rows[d].id, k, e, d, f)) || (c = {});
							}
							...
							if (a.p.cellsubmit == "remote") {
								if (a.p.cellurl) {
									...
									b("#lui_" + a.p.id).show();
									a.grid.hDiv.loading = true;
									b.ajax(b.extend({url:a.p.cellurl, data:b.isFunction(a.p.serializeCellData) ? a.p.serializeCellData.call(a, n) : n, type:"POST", complete:function (o, p) {
										b("#lui_" + a.p.id).hide();
										a.grid.hDiv.loading = false;
										if (p == "success") {
		...
		return this.each(function () {
			function d(g, i, k) {
				if (k.substr(0, 1) == "v") {
					...
					if (k == "vd") {
						if (l >= j) {
							b(a.grid.bDiv)[0].scrollTop = b(a.grid.bDiv)[0].scrollTop + a.rows[g].clientHeight;
						}
					}
					if (k == "vu") {
						if (q < m) {
				...
				if (k == "h") {
					...
					if (a.rows[g].cells[i].offsetLeft + a.rows[g].cells[i].clientWidth >= k + parseInt(j, 10)) {
						b(a.grid.bDiv)[0].scrollLeft = b(a.grid.bDiv)[0].scrollLeft + a.rows[g].cells[i].clientWidth;
					} else {
						if (m < j) {
							b(a.grid.bDiv)[0].scrollLeft = b(a.grid.bDiv)[0].scrollLeft - a.rows[g].cells[i].clientWidth;
						}
						...
						if (c !== "cb" && c !== "subgrid") {
							if (d == "dirty") {
								if (b(this).hasClass("dirty-cell")) {
									try {
										e[c] = b.unformat(this, {rowId:a.rows[h].id, colModel:a.p.colModel[g]}, g);
									}
									catch (i) {
...
(function (b) {
	...
	var l = 0, j = b.jqm.hash, k = [], q = b.browser.msie && b.browser.version == "6.0", e = false, r = function (a) {
...
(function (b) {
	...
	b.jqDnR = {dnr:{}, e:0, drag:function (a) {
		if (c.k == "d") {
			d.css({left:c.X + a.pageX - c.pX, top:c.Y + a.pageY - c.pY});
		} else {
	...
	var i = b.jqDnR, c = i.dnr, d = i.e, f, l = function (a, e, n, m) {
		return a.each(function () {
			...
			e.bind("mousedown", {e:a, k:n}, function (g) {
				...
				M1 = f && j.k != "d" ? {X:h.left || f1("left") || 0, Y:h.top || f1("top") || 0, W:f[0].offsetWidth || f1("width") || 0, H:f[0].offsetHeight || f1("height") || 0, pX:g.pageX, pY:g.pageY, k:j.k} : false;
...
(function (b) {
	b.jgrid.extend({setSubGrid:function () {
		return this.each(function () {
			...
			this.p.colNames.unshift("");
			this.p.colModel.unshift({name:"subgrid", width:b.browser.safari ? this.p.subGridWidth + this.p.cellLayout : this.p.subGridWidth, sortable:false, resizable:false, hidedlg:true, search:false, fixed:true});
		...
		return this.each(function () {
			...
			if (a.grid) {
				var n = function (g, j, e) {
						...
						for (h = 0; h < a.p.subGridModel[0].params.length; h++) {
							for (c = 0; c < a.p.colModel.length; c++) {
								if (a.p.colModel[c].name == a.p.subGridModel[0].params[h]) {
									e[a.p.colModel[c].name] = b("td:eq(" + c + ")", g).text().replace(/\&#160\;/ig, "");
								}
							}
						}
					}
					if (!a.grid.hDiv.loading) {
						...
						switch (a.p.subgridtype) {
						  case "xml":
						  case "json":
							b.ajax(b.extend({type:a.p.mtype, url:a.p.subGridUrl, dataType:a.p.subgridtype, data:b.isFunction(a.p.serializeSubGridData) ? a.p.serializeSubGridData.call(a, e) : e, complete:function (k) {
				...
				for (; r < y; ) {
					b(a.rows[r]).hasClass("jqgrow") && b(a.rows[r].cells[f]).bind("click", function () {
						...
						if (b(this).hasClass("sgcollapsed")) {
							...
							if (a.p.subGridOptions.reloadOnExpand === true || a.p.subGridOptions.reloadOnExpand === false && !b(q).hasClass("ui-subgrid")) {
								...
								if (b.isFunction(a.p.subGridBeforeExpand)) {
									p = a.p.subGridBeforeExpand.call(a, t + "_" + l, l);
								}
								if (p === false) {
									return false;
								}
							...
							if (b(this).hasClass("sgexpanded")) {
								...
								if (a.p.subGridOptions.reloadOnExpand === true) {
									...
									b(q).hasClass("ui-subgrid") && b(q).hide();
								}
		...
		return this.each(function () {
			if (this.grid || f) {
				if (this.p.subGrid === true) {
					...
					if (d) {
						(d = b("td.sgcollapsed", d)[0]) && b(d).trigger("click");
					}
				}
			}
		});
		...
		return this.each(function () {
			if (this.grid || f) {
				if (this.p.subGrid === true) {
					var d = b(this).jqGrid("getInd", f, true);
					if (d) {
						(d = b("td.sgexpanded", d)[0]) && b(d).trigger("click");
					}
				}
			}
		});
				...
				if (this.p.subGrid === true) {
					...
					if (d) {
						...
						if (a) {
							...
							(a = b("td.sgexpanded", d)[0]) && b(a).trigger("click");
						}
					}
				}
			}
...
(function (e) {
	e.jgrid.extend({groupingSetup:function () {
					...
					for (var c = 0; c < a.groupField.length; c++) {
						...
						if (a.groupSummary[c]) {
							...
							for (var b = this.p.colModel, d = 0, g = b.length; d < g; d++) {
								b[d].summaryType && a.summary[c].push({nm:b[d].name, st:b[d].summaryType, v:""});
							}
						}
					}
					this.p.scroll = false;
			...
			e.each(d.sortitems[0], function (o, n) {
				...
				if (d.groupSummary[0]) {
					...
					for (var p = b.p.colModel, q, s = a[n].length, j = 0; j < c; j++) {
						var t = "<td " + b.formatCol(j, 1, "") + ">&#160;</td>", u = "{0}";
						e.each(i, function () {
							if (this.nm == p[j].name) {
								if (p[j].summaryTpl) {
									u = p[j].summaryTpl;
		...
		return this.each(function () {
			...
			for (d = 0; d < b.groupField.length; d++) {
				!b.groupColumnShow[d] && b.visibiltyOnNextGrouping[d] && e(this).jqGrid("showCol", b.groupField[d]);
			}
			for (d = 0; d < a.length; d++) {
				b.visibiltyOnNextGrouping[d] = e("#" + this.p.id + "_" + a[d]).is(":visible");
			}
		...
		return this.each(function () {
			...
			if (a === true) {
				for (var c = this.p.groupingView, b = 0; b < c.groupField.length; b++) {
					!c.groupColumnShow[b] && c.visibiltyOnNextGrouping[b] && e(this).jqGrid("showCol", c.groupField);
				}
	...
	}, groupingCalculations:{sum:function (a, c, b) {
	...
	d.jgrid.extend({setTreeNode:function (a, c) {
		return this.each(function () {
			...
			if (b.grid && b.p.treeGrid) {
				for (var e = b.p.expColInd, g = b.p.treeReader.expanded_field, h = b.p.treeReader.leaf_field, f = b.p.treeReader.level_field, l = b.p.treeReader.icon_field, i = b.p.treeReader.loaded, j, o, n, k; a < c; ) {
					...
					if (k[i] !== undefined) {
						k[i] = k[i] == "true" || k[i] === true ? true : false;
					}
					...
					if (j !== parseInt(b.p.tree_root_level, 10)) {
						(k = (k = d(b).jqGrid("getNodeParent", k)) && k.hasOwnProperty(g) ? k[g] : true) || d(b.rows[a]).css("display", "none");
					}
		...
		return this.each(function () {
			...
			if (a.p.treeGrid) {
				...
				if (a.p.treeGridModel == "nested") {
					a.p.treeReader = d.extend({level_field:"level", left_field:"lft", right_field:"rgt", leaf_field:"isLeaf", expanded_field:"expanded", loaded:"loaded", icon_field:"icon"}, a.p.treeReader);
				} else {
					if (a.p.treeGridModel == "adjacency") {
						a.p.treeReader = d.extend({level_field:"level", parent_id_field:"parent", leaf_field:"isLeaf", expanded_field:"expanded", loaded:"loaded", icon_field:"icon"}, a.p.treeReader);
					}
				}
			...
			if (c.grid && c.p.treeGrid) {
				switch (c.p.treeGridModel) {
				  ...
				  case "adjacency":
					...
					d(c.p.data).each(function () {
						if (this[e] === null || String(this[e]).toLowerCase() == "null") {
							a.push(this);
						}
					});
				}
		...
		this.each(function () {
			if (this.grid && this.p.treeGrid) {
				switch (this.p.treeGridModel) {
				  case "nested":
					var b = this.p.treeReader.left_field, e = this.p.treeReader.right_field, g = this.p.treeReader.level_field, h = parseInt(a[b], 10), f = parseInt(a[e], 10), l = parseInt(a[g], 10);
		...
		this.each(function () {
			...
			if (this.grid && this.p.treeGrid) {
				switch (this.p.treeGridModel) {
				  case "nested":
					var e = this.p.treeReader.left_field, g = this.p.treeReader.right_field, h = this.p.treeReader.level_field, f = parseInt(a[e], 10), l = parseInt(a[g], 10), i = parseInt(a[h], 10);
					d(this.p.data).each(function () {
...
(function (b) {
	b.jgrid.extend({jqGridImport:function (a) {
		...
		return this.each(function () {
			...
			switch (a.imptype) {
			  case "xml":
				b.ajax(b.extend({url:a.impurl, type:a.mtype, data:a.impData, dataType:"xml", complete:function (f, g) {
					if (g == "success") {
						c(f.responseXML, a);
	...
	if (a.nodeType === 1) {
		if (a.attributes.length) {
			...
			for (e = 0; e < a.attributes.length; e += 1) {
				b["@" + a.attributes[e].nodeName] = (a.attributes[e].nodeValue || "").toString();
			}
		}
		if (a.firstChild) {
			var f = e = 0, d = false, c;
			for (c = a.firstChild; c; c = c.nextSibling) {
				if (c.nodeType === 1) {
					...
					if (c.nodeType === 3 && c.nodeValue.match(/[^ \f\n\r\t\v]/)) {
						e += 1;
					} else {
						if (c.nodeType === 4) {
							f += 1;
		...
		if (a instanceof Array) {
			...
			if (a === null) {
				...
				if (typeof a === "object") {
					...
					f += typeof a === "string" ? (b && ":") + "\"" + a.replace(/\\/g, "\\\\").replace(/\"/g, "\\\"") + "\"" : (b && ":") + "\"" + a.toString() + "\"";
				}
			}
		}
	}
		...
		var h = function (e) {
			...
			if (e.nodeType === 1) {
				...
				for (d = 0; d < e.attributes.length; d += 1) {
					f += " " + e.attributes[d].nodeName + "=\"" + (e.attributes[d].nodeValue || "").toString() + "\"";
				}
				if (e.firstChild) {
					f += ">";
		...
		if (!this.grid) {
			...
			jQuery("th", jQuery(this)).each(function () {
				if (c.length === 0 && l) {
					...
					c.push({name:jQuery(this).attr("id") || jQuery.trim(jQuery.jgrid.stripHtml(jQuery(this).html())).split(" ").join("_"), index:jQuery(this).attr("id") || jQuery.trim(jQuery.jgrid.stripHtml(jQuery(this).html())).split(" ").join("_"), width:jQuery(this).width() || 150});
...
(function (a) {
	...
	a.jgrid.extend({sortableColumns:function (b) {
		return this.each(function () {
			...
			var c = this, f = c.p.id;
			f = {tolerance:"pointer", axis:"x", scrollSensitivity:"1", items:">th:not(:has(#jqgh_" + f + "_cb,#jqgh_" + f + "_rn,#jqgh_" + f + "_subgrid),:hidden)", placeholder:{element:function (g) {
		...
		function h(d, k) {
			if (d) {
				if (typeof d == "string") {
					a.fn[d] && a.fn[d].apply(k, a.makeArray(arguments).slice(2));
				} else {
					a.isFunction(d) && d.apply(k, a.makeArray(arguments).slice(2));
				}
			}
		}
		var c = this;
		if (!a("#colchooser_" + c[0].p.id).length) {
			...
			b = a.extend({width:420, height:240, classname:null, done:function (d) {
			...
			var g = c.jqGrid("getGridParam", "colModel"), j = c.jqGrid("getGridParam", "colNames"), i = {}, l = [];
			e.empty();
			a.each(g, function (d) {
				i[this.name] = d;
				if (this.hidedlg) {
					...
					e.append("<option value='" + d + "' " + (this.hidden ? "" : "selected='selected'") + ">" + j[d] + "</option>");
				}
			});
			...
			if (c.grid) {
				if (!c.p.treeGrid) {
					if (a.fn.draggable && a.fn.droppable) {
						...
						if (typeof b == "string" && b == "updateDnD" && c.p.jqgdnd === true) {
							h();
						} else {
							b = a.extend({drag:function (e) {
								return a.extend({start:function (g, j) {
									if (c.p.subGrid) {
									...
									for (i = 0; i < a.data(c, "dnd").connectWith.length; i++) {
										a(a.data(c, "dnd").connectWith[i]).jqGrid("getGridParam", "reccount") == "0" && a(a.data(c, "dnd").connectWith[i]).jqGrid("addRowData", "jqg_empty_row", {});
									}
									...
									for (i = 0; i < a.data(c, "dnd").connectWith.length; i++) {
										a(a.data(c, "dnd").connectWith[i]).jqGrid("delRowData", "jqg_empty_row");
									}
									e.onstop && a.isFunction(e.onstop) && e.onstop.call(a(c), g, j);
								}}, e.drag_opts || {});
							}, drop:function (e) {
								return a.extend({accept:function (g) {
									...
									if (a(j.draggable).hasClass("jqgrow")) {
										...
										if (!e.dropbyname) {
											var l = 0, m = {}, d, k = a("#" + this.id).jqGrid("getGridParam", "colModel");
											try {
												for (var p in i) {
										...
										if (e.beforedrop && a.isFunction(e.beforedrop)) {
											...
											if (typeof d != "undefined" && d !== null && typeof d == "object") {
												i = d;
											}
										}
										if (j.helper.dropped) {
											var n;
											if (e.autoid) {
												if (a.isFunction(e.autoid)) {
													n = e.autoid.call(this, i);
												} else {
							...
							}, onstart:null, onstop:null, beforedrop:null, ondrop:null, drop_opts:{activeClass:"ui-state-active", hoverClass:"ui-state-hover"}, drag_opts:{revert:"invalid", helper:"clone", cursor:"move", appendTo:"#jqgrid_dnd", zIndex:5000}, dragcopy:false, dropbyname:false, droppos:"first", autoid:true, autoidprefix:"dnd_"}, b || {});
							if (b.connectWith) {
								...
								for (var f = 0; f < b.connectWith.length; f++) {
									a(b.connectWith[f]).droppable(a.isFunction(b.drop) ? b.drop.call(a(c), b) : b.drop);
								}
							}
						}
					}
